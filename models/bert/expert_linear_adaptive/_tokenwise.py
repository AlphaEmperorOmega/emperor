...
@dataclass
class BertTokenwiseLinearLayerConfig(ConfigBase):
    ...
    def _registry_owner(self) -> type:
        return BertTokenwiseLinearLayer


class BertTokenwiseLinearLayer(Module):
    def __init__(
        self,
        cfg: "BertTokenwiseLinearLayerConfig",
        overrides: "BertTokenwiseLinearLayerConfig | LinearLayerConfig | None" = None,
    ) -> None:
        super().__init__()
    ...
    def forward(self, input_batch: Tensor) -> Tensor:
        if input_batch.dim() < 2:
            raise ValueError(
                "BertTokenwiseLinearLayer expects at least 2D input with a feature "
