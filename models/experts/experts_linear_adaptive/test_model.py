...
class TestExpertsLinearAdaptiveModel(unittest.TestCase):
    ...
    def test_option_group_build_matches_flat_kwargs(self):
        def controller_stack_kwargs(
            ...
            *,
            ...
            if include_hidden:
                values[f"{prefix}_hidden_dim"] = options.hidden_dim
            return values
        ...
        flat_builder = ExpertsLinearAdaptiveConfigBuilder(**flat_kwargs)
        grouped_builder = ExpertsLinearAdaptiveConfigBuilder(
            batch_size=3,
            learning_rate=0.02,
            input_dim=8,
            output_dim=4,
    ...
    def test_disabled_memory_is_absent_from_stack_and_layer_configs(self):
        cfg = ExpertsLinearAdaptiveConfigBuilder().build()
        stack_cfg = cfg.experiment_config.model_config.stack_config
    ...
    def test_memory_config_is_shared_on_stack_config(self):
        cfg = ExpertsLinearAdaptiveConfigBuilder(memory_flag=True).build()
        stack_cfg = cfg.experiment_config.model_config.stack_config
    ...
    def test_memory_config_uses_overrides_when_independent(self):
        ...
        self.assertIsInstance(
            memory_stack.layer_config.layer_model_config,
            LinearLayerConfig,
        )
            ...
            with self.subTest(
                ...
                if "router_weight" in case:
                    router_augmentation_config = (
                        moe_model_cfg.sampler_config.router_config.model_config
