...
class CoefficientOfVariationLoss(AuxiliaryLossBase):
    ...
    def __compute_coefficient_of_variation(self) -> Tensor:
        denominator = torch.linalg.vector_norm(
            self.gates_accumulation, ord=1
        ).clamp_min(1e-12)
        probabilities = self.gates_accumulation / denominator
...
class SwitchLoss(AuxiliaryLossBase):
    ...
    def __compute_switch_loss(self) -> Tensor:
        ...
        probability_norm = torch.linalg.vector_norm(p, ord=1).clamp_min(1e-12)
        frequency_norm = f.sum().clamp_min(1)
        normalized_probabilities = p / probability_norm
        normalized_frequency = f / frequency_norm
...
class ZeroCentredLoss(AuxiliaryLossBase):
    ...
    def update_accumulation(self, logits: Tensor | None = None) -> None:
        ...
        count = logits.new_ones((), dtype=torch.long) * logits.size(0)
...
class MutualInformationLoss(AuxiliaryLossBase):
    ...
    def __compute_mutual_information_loss(self):
        probabilities = torch.vstack(self.probabilities)
        log_probabilities = torch.vstack(self.log_probabilities)
        masks = torch.vstack(self.skip_masks)
