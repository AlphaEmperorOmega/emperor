...
class ClusterPlasticityDelegate:
    """Apply growth and pruning while keeping counters on the owner."""
    ...
    def capture_growth_counter_baseline(self) -> _GrowthCounterBaseline | None:
        if self.__owner.growth_threshold is None:
            return None
        neuron_names = tuple(sorted(self.__owner.cluster.keys()))
        return _GrowthCounterBaseline(
            neuron_names=neuron_names,
            batch_counters=torch.stack(
                [self.__owner.cluster[name].batch_counter for name in neuron_names]
            ).clone(),
            escape_counts=(
                None
                if self.__owner.escape_counts is None
                else self.__owner.escape_counts.clone()
            ),
        )

    def mark_growth_counters_global_after_load(self, _module, _incompatible_keys):
        self.__owner._growth_counters_are_global = True

    def check_neuron_growth(
        self,
        ...
        for (
            ...
            self.__grow_neuron_at_position(
                growth_position,
                saturated_neuron,
                forward_context,
            )
    ...
    def __synchronize_batch_counters_across_ranks(
        ...
        sorted_neuron_names = tuple(sorted(self.__owner.cluster.keys()))
        stacked_batch_counters = torch.stack(
            [
                self.__owner.cluster[neuron_name].batch_counter
        ...
        if self.__is_distributed_training_initialized():
            ...
            for neuron_name, counter in zip(
                ...
                self.__owner.cluster[neuron_name].batch_counter.copy_(counter)
        return {
    ...
    def __batch_counter_baseline(
        ...
        if not self.__owner._growth_counters_are_global:
            return None
        if (
    ...
    def __synchronize_escape_counts_across_ranks(
        ...
        if not self.__is_distributed_training_initialized():
            return self.__owner.escape_counts
        escape_count_baseline = None
        if self.__owner._growth_counters_are_global:
            if (
                growth_counter_baseline is None
        ...
        self.__owner.escape_counts.copy_(synchronized_escape_counts)
        return synchronized_escape_counts

    def __has_exhausted_growth_budget(self) -> bool:
        if self.__owner.total_growth_count is None:
            return False
        return int(self.__owner.total_growth_count) >= self.__owner.max_total_growths

    def __is_within_growth_cooldown_after_counting_forward(self) -> bool:
        if self.__owner.forwards_since_last_growth is None:
            return False
        self.__owner.forwards_since_last_growth += 1
        return (
            int(self.__owner.forwards_since_last_growth)
            < self.__owner.growth_cooldown_steps
        )
    ...
    def __saturated_neurons_by_descending_counter(
        ...
        saturated_neurons = [
            ...
            if synchronized_batch_counters[neuron_name] >= self.__owner.growth_threshold
        ]
        return sorted(
            saturated_neurons,
    ...
    def __find_closest_empty_connection(
        ...
        origin_x, origin_y, origin_z = self.__topology.parse_neuron_name(neuron_name)

        empty_connection_positions = []
        for connection_coordinate_row in connection_coordinate_rows:
            connection_position = self.__topology.coordinate_from_row(
                connection_coordinate_row
            )
            if not self.__topology.is_within_grid_capacity(connection_position):
                continue
            candidate_neuron_name = self.__topology.neuron_name(*connection_position)
            if candidate_neuron_name not in self.__owner.cluster:
    ...
    def __grow_neuron_at_position(
        self,
        growth_position: tuple[int, int, int],
        saturated_neuron: Module,
        forward_context: _NeuronClusterForwardContext,
    ) -> None:
        x, y, z = growth_position
        grown_neuron_name = self.__topology.neuron_name(x, y, z)
        self.__owner._add_neuron(
            self.__owner.cluster,
            grown_neuron_name,
        ...
        self.__start_grown_neuron_warmup(self.__owner.cluster[grown_neuron_name])
    ...
    def __initialize_grown_neuron(
        ...
        grown_neuron = self.__owner._initialize_neuron(
            x,
            y,
            z,
            runtime_template=parent_neuron,
        )
        if not self.__owner.mitosis_initialization_flag:
            return grown_neuron
    ...
    def __copy_and_perturb_parent_parameters(
        ...
        with torch.no_grad():
            for parameter_name, grown_parameter in grown_parameters.items():
                ...
                self.__perturb_copied_parameter(grown_parameter, parent_parameter)
    ...
    @staticmethod
    def __perturb_copied_parameter(
        grown_parameter: Tensor,
        parent_parameter: Tensor,
    ) -> None:
        if grown_parameter.is_floating_point() or grown_parameter.is_complex():
            ...
            parameter_standard_deviation = statistics_source_parameter.std(correction=0)
    ...
    def __start_grown_neuron_warmup(self, neuron: Module) -> None:
        ...
        neuron.register_buffer(
            ...
            torch.tensor(
                self.__owner.growth_warmup_steps,
                dtype=torch.int64,
    ...
    def __reset_escape_count(self, growth_position: tuple[int, int, int]) -> None:
        if self.__owner.escape_counts is None:
            return
        x, y, z = growth_position
        self.__owner.escape_counts[x - 1, y - 1, z - 1] = 0

    def __record_successful_growth(self) -> None:
        if self.__owner.forwards_since_last_growth is not None:
            self.__owner.forwards_since_last_growth.zero_()
        if self.__owner.total_growth_count is not None:
            self.__owner.total_growth_count += 1

    def advance_grown_neuron_warmup(self) -> None:
        if self.__owner.growth_warmup_steps is None:
            return
        for neuron in self.__owner.cluster.values():
    ...
    def record_escaped_missing_positions(
        self,
        escaped_positions: list[tuple[int, int, int]],
    ) -> None:
        if (
            self.__owner.escape_counts is None
            or not self.__owner.training
            or not escaped_positions
        ):
            return
        zero_based_coordinates = (
            torch.tensor(
                escaped_positions,
                dtype=torch.long,
                device=self.__owner.escape_counts.device,
            )
            - 1
        )
        flattened_coordinate_indices = (
            zero_based_coordinates[:, 0] * self.__owner.y_axis_total_neurons
            + zero_based_coordinates[:, 1]
        ) * self.__owner.z_axis_total_neurons + zero_based_coordinates[:, 2]
        self.__owner.escape_counts.view(-1).index_add_(
            0,
            flattened_coordinate_indices,
            torch.ones_like(flattened_coordinate_indices),
        )

    def check_neuron_atrophy(
        self,
        forward_context: _NeuronClusterForwardContext,
    ) -> None:
        if self.__owner.pruning_threshold is None:
            return
        ...
        if prunable_neuron_name is None:
            return
        del self.__owner.cluster[prunable_neuron_name]
    ...
    def __synchronize_atrophy_counters_across_ranks(self) -> dict[str, int]:
        sorted_neuron_names = sorted(self.__owner.cluster.keys())
        stacked_atrophy_counters = torch.stack(
            [
                self.__owner.cluster[neuron_name].atrophy_counter
                for neuron_name in sorted_neuron_names
            ]
        )
        if self.__is_distributed_training_initialized():
            ...
            for neuron_name, counter in zip(
                ...
                self.__owner.cluster[neuron_name].atrophy_counter.copy_(counter)
        return {
            neuron_name: int(counter.item())
            for neuron_name, counter in zip(
                sorted_neuron_names,
    ...
    def __most_atrophied_prunable_neuron(
        ...
        prunable_neuron_names = [
            ...
            and synchronized_atrophy_counters[neuron_name]
            >= self.__owner.pruning_threshold
        ]
    ...
    def __entry_plane_neuron_names(self) -> set[str]:
        return {
            self.__topology.neuron_name(int(x), int(y), int(z))
            for x, y, z in self.__owner.entry_coordinates.tolist()
        }
