...
class MixtureOfAttentionHeadsProcessor(ProcessorBase):
    ...
    def _compute_raw_masked_attention_logits(
        ...
        runtime_layout = scaled_inputs.runtime_layout
        ...
        source_sequence_length = scaled_inputs.key.size(-2)

        transposed_key = scaled_inputs.key.transpose(-2, -1)
        if self.use_kv_expert_models_flag:
            attention_logits = torch.matmul(scaled_inputs.query, transposed_key)
        else:
        ...
        attention_logits = attention_logits.contiguous().view(
            total_batch_size,
    ...
    def _compute_normalized_attention_weights(
        ...
        safe_attention_logits = raw_attention_logits.masked_fill(
            fully_masked_rows,
            0.0,
        )
        normalized_attention_weights = F.softmax(safe_attention_logits, dim=-1)
        return normalized_attention_weights.masked_fill(
            fully_masked_rows,
            0.0,
        )
