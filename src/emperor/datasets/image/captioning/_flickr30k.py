...
class Flickr30k(DataModule):
    ...
    def __init__(
        ...
        train_ann_file: str = "data/flickr30k/captions_train.token",
        val_ann_file: str = "data/flickr30k/captions_val.token",
    ):
    ...
    def prepare_data(self) -> None:
        pass  # Flickr30k requires manual download

    def _setup_fit(self) -> None:
        raw_train = datasets.Flickr30k(
            root=self.root,
            ann_file=self.train_ann_file,
            transform=self._get_transforms(),
        )
        self._build_vocab(raw_train)
        raw_val = datasets.Flickr30k(
            root=self.root,
            ann_file=self.val_ann_file,
            transform=self._get_transforms(),
        )
        self.train = _CaptioningDataset(
            raw_train, self.tokenizer, self.vocab, self.sequence_length, train=True
        )
        self.val = _CaptioningDataset(
            raw_val, self.tokenizer, self.vocab, self.sequence_length, train=False
        )
    ...
    def _build_vocab(self, dataset) -> None:
        ...
        Flickr30k.vocab_size = len(self.vocab)
        Flickr30k.num_classes = len(self.vocab)
