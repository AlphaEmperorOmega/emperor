...
from torchtext.datasets import AG_NEWS as AgNewsDataset
...
class AgNews(DataModule):
    vocab_size: int = (
        95812  # approximate AG_NEWS vocab size with basic_english tokenizer
    )
    num_classes: int = 4
    ...
    def prepare_data(self) -> None:
        AgNewsDataset(root=self.root, split="train")
        AgNewsDataset(root=self.root, split="test")

    def _setup_fit(self) -> None:
        self._build_vocab()
        train_iter = AgNewsDataset(root=self.root, split="train")
        val_iter = AgNewsDataset(root=self.root, split="test")
    ...
    def _build_vocab(self) -> None:
        ...
        AgNews.vocab_size = len(self.vocab)
        AgNews.flattened_input_dim = len(self.vocab)
