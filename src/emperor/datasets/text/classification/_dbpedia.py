...
from torchtext.datasets import DBpedia as DBpediaDataset
...
class DBpedia(DataModule):
    vocab_size: int = (
        193397  # approximate DBpedia vocab size with basic_english tokenizer
    )
    num_classes: int = 14
    ...
    def prepare_data(self) -> None:
        DBpediaDataset(root=self.root, split="train")
        DBpediaDataset(root=self.root, split="test")

    def _setup_fit(self) -> None:
        self._build_vocab()
        train_iter = DBpediaDataset(root=self.root, split="train")
        val_iter = DBpediaDataset(root=self.root, split="test")
