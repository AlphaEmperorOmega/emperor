...
def _encode(text, tokenizer, vocab, sequence_length):
    tokens = (
        [vocab["<bos>"]]
        + vocab(tokenizer(text))[: sequence_length - 2]
        + [vocab["<eos>"]]
    )
...
class _SummarizationDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        summary = torch.tensor(
            _encode(
                sample["highlights"], self.tokenizer, self.vocab, self.summary_length
            ),
            dtype=torch.long,
        )
...
class CnnDailyMail(DataModule):
    ...
    def _setup_fit(self) -> None:
        ...
        self.train = _SummarizationDataset(
            train_data,
            self.tokenizer,
            self.vocab,
            self.article_length,
        ...
        self.val = _SummarizationDataset(
            val_data,
            self.tokenizer,
            self.vocab,
            self.article_length,
