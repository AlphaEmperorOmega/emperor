...
class _NLIDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        premise = torch.tensor(
            _encode(
                sample["premise"], self.tokenizer, self.vocab, self.sequence_length
            ),
            dtype=torch.long,
        )
        hypothesis = torch.tensor(
            _encode(
                sample["hypothesis"], self.tokenizer, self.vocab, self.sequence_length
            ),
            dtype=torch.long,
        )
...
class MultiNLI(DataModule):
    vocab_size: int = 84659  # approximate MultiNLI vocab size
    ...
    def prepare_data(self) -> None:
        load_dataset("multi_nli", split="train")
        load_dataset("multi_nli", split="validation_matched")

    def _setup_fit(self) -> None:
        train_data = list(load_dataset("multi_nli", split="train"))
        val_data = list(load_dataset("multi_nli", split="validation_matched"))
        self._build_vocab(train_data)
        self.train = _NLIDataset(
            train_data, self.tokenizer, self.vocab, self.sequence_length
        )
        self.val = _NLIDataset(
            val_data, self.tokenizer, self.vocab, self.sequence_length
        )
