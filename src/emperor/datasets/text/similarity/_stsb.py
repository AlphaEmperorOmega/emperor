...
class _STSDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sentence1 = torch.tensor(
            _encode(
                sample["sentence1"], self.tokenizer, self.vocab, self.sequence_length
            ),
            dtype=torch.long,
        )
        sentence2 = torch.tensor(
            _encode(
                sample["sentence2"], self.tokenizer, self.vocab, self.sequence_length
            ),
            dtype=torch.long,
        )
