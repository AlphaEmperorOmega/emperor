...
from models.gpt.expert_linear._gate_config_factory import GateConfigFactory
from models.gpt.expert_linear._halting_config_factory import HaltingConfigFactory
from models.gpt.expert_linear._memory_config_factory import MemoryConfigFactory
from models.gpt.expert_linear._recurrent_config_factory import RecurrentConfigFactory


class GptGateConfigFactory:
    def __init__(self, dependencies: GptControlFactoryDependencies) -> None:
        self.dependencies = dependencies

    def build_decoder_factory(self) -> GateConfigFactory | None:
        dependencies = self.dependencies
        if (
            dependencies.decoder_layer_controller_options is None
            or dependencies.decoder_recurrent_controller_options is None
            or dependencies.decoder_submodule_stack_options is None
        ):
            return None
        return GateConfigFactory(
            layer_controller_options=dependencies.decoder_layer_controller_options,
            recurrent_controller_options=dependencies.decoder_recurrent_controller_options,
            submodule_stack_options=dependencies.decoder_submodule_stack_options,
        )
...
class GptHaltingConfigFactory:
    def __init__(self, dependencies: GptControlFactoryDependencies) -> None:
        self.dependencies = dependencies

    def build_decoder_factory(self) -> HaltingConfigFactory | None:
        dependencies = self.dependencies
        if (
            dependencies.decoder_layer_controller_options is None
            or dependencies.decoder_recurrent_controller_options is None
...
class GptMemoryConfigFactory:
    def __init__(self, dependencies: GptControlFactoryDependencies) -> None:
        self.dependencies = dependencies

    def build_decoder_factory(self) -> MemoryConfigFactory | None:
        dependencies = self.dependencies
        if (
            dependencies.decoder_dynamic_memory_options is None
            or dependencies.decoder_submodule_stack_options is None
        ):
            return None
        return MemoryConfigFactory(
            hidden_dim=dependencies.hidden_dim,
            stack_options=dependencies.decoder_stack_options,
            dynamic_memory_options=dependencies.decoder_dynamic_memory_options,
            submodule_stack_options=dependencies.decoder_submodule_stack_options,
        )
...
class GptRecurrentConfigFactory:
    def __init__(self, dependencies: GptControlFactoryDependencies) -> None:
        self.dependencies = dependencies
        self.gate_factory = GptGateConfigFactory(dependencies)
        self.halting_factory = GptHaltingConfigFactory(dependencies)

    def build_decoder_factory(self) -> RecurrentConfigFactory | None:
        dependencies = self.dependencies
        if dependencies.decoder_recurrent_controller_options is None:
            return None
        gate_factory = self.gate_factory.build_decoder_factory()
        halting_factory = self.halting_factory.build_decoder_factory()
