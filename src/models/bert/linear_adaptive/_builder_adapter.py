...
def _bert_builder_kwargs(
    ...
    for builder_key, factory, config_attr, factory_kwargs in (
        ...
        (
            "mlm_head_options",
            _mlm_head_options_from_kwargs,
            "MLM_HEAD_OPTIONS",
            {},
        ),
        (
            "nsp_head_options",
            _nsp_head_options_from_kwargs,
            "NSP_HEAD_OPTIONS",
            {},
        ),
        (
...
def _mlm_head_options_from_kwargs(options, kwargs: dict[str, Any]):
    return replace(
        options,
        **_updates(
            kwargs,
...
def _nsp_head_options_from_kwargs(options, kwargs: dict[str, Any]):
    return replace(
        options,
        **_updates(
            kwargs,
...
def _default_config_options(config_module: ModuleType, config_attr: str) -> Any:
    option_factories: dict[str, Callable[[ModuleType], Any]] = {
        "EMBEDDING_OPTIONS": config_defaults.bert_embedding_options,
        "ENCODER_OPTIONS": config_defaults.bert_encoder_options,
        "POSITIONAL_EMBEDDING_OPTIONS": (
            config_defaults.bert_positional_embedding_options
        ),
        "ATTENTION_OPTIONS": config_defaults.bert_attention_options,
        "FEED_FORWARD_OPTIONS": config_defaults.bert_feed_forward_options,
        "MLM_HEAD_OPTIONS": config_defaults.bert_mlm_head_options,
        "NSP_HEAD_OPTIONS": config_defaults.bert_nsp_head_options,
