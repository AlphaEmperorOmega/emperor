...
from models.bert.expert_linear._control_factory_dependencies import (
    BertControlFactoryDependencies,
)
from models.bert.expert_linear._encoder_control_config_factory import (
    BertGateConfigFactory,
    BertHaltingConfigFactory,
    BertMemoryConfigFactory,
    BertRecurrentConfigFactory,
)
from models.bert.expert_linear._linear_layer_config_factory import (
    LinearLayerConfigFactory as BertLinearLayerConfigFactory,
)
...
class _BertExpertConfigFactory(Protocol):
...
class CoreConfigDependencies:
    ...
    linear_layer_config_factory: BertLinearLayerConfigFactory
    attention_projection_linear_layer_config_factory: (
        BertLinearLayerConfigFactory | None
    ) = None
    feed_forward_linear_layer_config_factory: BertLinearLayerConfigFactory | None = None
    expert_config_factory: _BertExpertConfigFactory | None = None


class BertCoreConfigFactory:
    ...
    def _apply_attention_projection_controls(
        ...
        dependencies = self._control_factory_dependencies()
        gate_factory = BertGateConfigFactory(
            dependencies
        ).build_attention_projection_factory()
        halting_factory = BertHaltingConfigFactory(
            dependencies
        ).build_attention_projection_factory()
        memory_factory = BertMemoryConfigFactory(
            dependencies
        ).build_attention_projection_factory()
    ...
    def _apply_feed_forward_controls(self, stack_config) -> None:
        ...
        dependencies = self._control_factory_dependencies()
        gate_factory = BertGateConfigFactory(dependencies).build_feed_forward_factory()
        halting_factory = BertHaltingConfigFactory(
            dependencies
        ).build_feed_forward_factory()
        memory_factory = BertMemoryConfigFactory(
    ...
    def _stack_residual_block_size(self) -> int | None:
        return self.encoder_stack_options.residual_block_size

    def _stack_residual_rms_norm_epsilon(self) -> float | None:
        return self.encoder_stack_options.residual_rms_norm_epsilon
    ...
    def _stack_apply_output_postprocessing_flag(self) -> bool:
        return self.encoder_stack_options.apply_output_postprocessing_flag
    ...
    def _control_factory_dependencies(self) -> BertControlFactoryDependencies:
        return BertControlFactoryDependencies(
            hidden_dim=self.hidden_dim,
            encoder_stack_options=self.encoder_stack_options,
