...
from . import config
from ._runtime_defaults_resolver import _NeuronLinearRuntimeDefaultsResolver
...
def _raise_outer_unknown_key(
    ...
    for key in values:
        if repr(key) in message:
            raise TypeError(
                f"_NeuronLinearRuntimeDefaultsResolver.__init__() got an unexpected keyword argument {key!r}"
            ) from None
    raise error
...
def runtime_from_flat(values: Mapping[str, object] | None = None) -> RuntimeOptions:
    ...
    validate_runtime_default_value_types(
        flat_values,
        package="models.neuron.linear",
        config_module=config,
    )
    try:
        resolver = _NeuronLinearRuntimeDefaultsResolver(flat_values)
