...
def _normalize_overrides(overrides: Mapping[str, object]) -> dict[str, object]:
    ...
    for supplied_key, value in overrides.items():
        ...
        key = supplied_key.strip().replace("-", "_").lower()
        runtime_field = _FIELDS.get(key)
        ...
        if key in normalized:
            raise ValueError(
                f"{_PACKAGE}: conflicting normalized runtime keys "
                f"{sources[key]!r} and {supplied_key!r}"
            )
        if not runtime_field.accepts(value):
            _raise_type_error(supplied_key, value, runtime_field.expected)
        normalized[key] = value
        sources[key] = supplied_key
    return normalized
