...
def runtime_from_builder_options(
    ...
    unknown = set(builder_options) - set(_LEGACY_BUILDER_FIELDS)
    if unknown:
        name = sorted(unknown)[0]
        raise TypeError(
            "LinearConfigBuilder.__init__() got an unexpected keyword "
    ...
    return runtime_from_flat(builder_options, config)


def runtime_from_legacy_options(
    *args: Any,
    **kwargs: Any,
) -> options.RuntimeOptions:
    if len(args) > len(_LEGACY_BUILDER_FIELDS):
        raise TypeError(
            "LinearConfigBuilder.__init__() takes at most "
    ...
    for name, value in zip(_LEGACY_BUILDER_FIELDS, args, strict=False):
        ...
        builder_options[name] = value
    return runtime_from_builder_options(builder_options)
