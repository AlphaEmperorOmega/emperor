...
class Model(TranslationExperiment):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.source_embedding = shared_embedding
        self.target_embedding = shared_embedding
    ...
    def forward(
        ...
        encoder_output, source_padding_mask, encoder_loss = self._encode(source_ids)
        decoder_output, decoder_loss = self._decode(
            target_input_ids,
            encoder_output,
            source_padding_mask,
        )
        auxiliary_loss = (encoder_loss + decoder_loss).reshape(())
        return self.output_projection(decoder_output), auxiliary_loss

    def _embed_source(self, source_ids: Tensor) -> Tensor:
        hidden = self.source_embedding(source_ids) * self.embedding_scale
        hidden = hidden + self.source_positional_embedding(source_ids)
        return self.embedding_dropout(hidden)

    def _embed_target(self, target_ids: Tensor) -> Tensor:
        hidden = self.target_embedding(target_ids) * self.embedding_scale
        hidden = hidden + self.target_positional_embedding(target_ids)
        return self.embedding_dropout(hidden)

    def _encode(self, source_ids: Tensor) -> tuple[Tensor, Tensor, Tensor]:
        self._validate_ids(source_ids, self.source_sequence_length, "source")
        source_ids = source_ids.to(self.device, dtype=torch.long)
        source_padding_mask = source_ids.eq(self.pad_token_id)
        state = self.encoder(
            AttentionLayerState(
                hidden=self._embed_source(source_ids),
                key_padding_mask=source_padding_mask,
            )
        )
        output = self.encoder_layer_norm(state.hidden)
        loss = state.loss if state.loss is not None else output.new_zeros(())
        return output, source_padding_mask, loss
    ...
    def _decode(
        ...
        target_padding_mask = target_ids.eq(self.pad_token_id)
        state = self.decoder(
            TransformerDecoderLayerState(
                hidden=self._embed_target(target_ids),
    ...
    def _validate_ids(self, ids: Tensor, maximum_length: int, name: str) -> None:
        ...
        if ids.size(1) > maximum_length:
            raise ValueError(
                f"{name} sequence length {ids.size(1)} exceeds maximum "
                f"{maximum_length}."
            )
    ...
    def generate(
        ...
        try:
            self.eval()
            source_ids = source_ids.to(self.device, dtype=torch.long)
            encoder_output, source_padding_mask, _ = self._encode(source_ids)
            ...
            for position in range(1, maximum):
                decoder_output, _ = self._decode(
                    generated[:, :position],
                    encoder_output,
                    source_padding_mask,
                )
                next_ids = self.output_projection(decoder_output[:, -1]).argmax(dim=-1)
