...
export function useCompareWorkspaceState({
  ...
  const models = target.modelsQuery.data?.models ?? emptyModels;
  ...
  return {
    catalog: {
      isLoading: target.modelsQuery.isLoading,
      isError: target.modelsQuery.isError,
      error: target.modelsQuery.error,
    },
    modelOptions,
    entries: entryData,
