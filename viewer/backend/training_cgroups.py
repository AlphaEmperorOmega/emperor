"""Linux cgroup-v2 helpers for strict training job containment."""
...
CGROUP_NAMESPACE = "emperor-viewer-training"
...
class CgroupV2Manager:
    ...
    def create_job_cgroup(self, job_id: str) -> CgroupV2Job:
        ...
        job_path = namespace_path / f"job-{job_id}"
        try:
    ...
    def from_existing(self, cgroup_path: str | None) -> CgroupV2Job | None:
        if not cgroup_path:
            return None
        path = Path(cgroup_path)
        if not path.exists():
            return None
        return CgroupV2Job(path)
