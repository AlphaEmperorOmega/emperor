...
class WikiText103(DataModule):
    vocab_size: int = 267735  # standard WikiText103 vocab size with basic_english tokenizer
    ...
    def prepare_data(self) -> None:
        WikiText103Dataset(root=self.root, split="train")
        WikiText103Dataset(root=self.root, split="valid")

    def _setup_fit(self) -> None:
        self._build_vocab()
        train_iter = WikiText103Dataset(root=self.root, split="train")
        val_iter = WikiText103Dataset(root=self.root, split="valid")
