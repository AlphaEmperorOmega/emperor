...
class _QADataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # answer span: start/end token indices within the context (clamped to context_length)
        start = min(sample["answers"]["answer_start"][0], self.context_length - 1) if sample["answers"]["answer_start"] else 0
        end = min(start + len(self.tokenizer(sample["answers"]["text"][0])) - 1, self.context_length - 1) if sample["answers"]["text"] else 0
...
class SQuADv1(DataModule):
    ...
    def _build_vocab(self, data) -> None:
        ...
        SQuADv1.vocab_size = len(self.vocab)
