...
class _MemoryDiagnostics:
    ...
    def calculate_gate(
        ...
        from emperor.memory.core.variants.weighted import WeightedDynamicMemory

        detached_logits = gate_logits.detach().float()
        if isinstance(memory_module, WeightedDynamicMemory):
            memory_share = torch.softmax(detached_logits, dim=-1)[..., -1]
    ...
    def __make_memory_forward_hook(
        ...
        def log_memory_output(
            ...
            global_step = getattr(
                trainer,
                "global_step",
                getattr(pl_module, "global_step", 0),
            )
