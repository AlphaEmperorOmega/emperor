...
def _validated_process(
    spec: ServiceSpec,
    *,
    require_current_command: bool = True,
):
    import psutil
    ...
    if metadata is None:
        return None
    try:
        recorded_command = metadata.get("argv")
        if not isinstance(recorded_command, list) or not all(
            isinstance(argument, str) for argument in recorded_command
        ):
            return None
        if (
            metadata.get("commandIdentity") != spec.command_identity
            or int(metadata.get("port")) != spec.port
            or (require_current_command and recorded_command != list(spec.command))
        ):
            return None
        process = psutil.Process(int(metadata["pid"]))
        observed_creation = process.create_time()
        expected_creation = float(metadata["createTime"])
        command = " ".join(process.cmdline())
        if abs(observed_creation - expected_creation) > 0.01:
            return None
        if spec.command_identity.casefold() not in command.casefold():
            return None
        return process
...
def _legacy_command_matches(spec: ServiceSpec, command: list[str]) -> bool:
    ...
    if spec.name == "backend":
        identity = ["-m", "uvicorn", "workbench.backend.api:app"]
        identity_matches = any(
            folded[index : index + len(identity)] == identity
            for index in range(len(folded) - len(identity) + 1)
        )
        port_matches = any(
            argument == f"--port={spec.port}"
            or (
                argument == "--port"
                and index + 1 < len(command)
                and command[index + 1] == str(spec.port)
            )
            for index, argument in enumerate(folded)
        )
        return identity_matches and port_matches
...
def _start_service(spec: ServiceSpec) -> None:
    ...
    existing = _validated_process(spec)
    if existing is not None:
        if _http_ready(spec.ready_url):
            print(
                f"Workbench {spec.name}: running (PID {existing.pid}, port {spec.port})"
            )
            return
        _wait_ready(spec, existing)
        return
    _retire_legacy_service(spec)
    stale_metadata = _read_json(spec.metadata_path)
    if os.name == "nt" and stale_metadata is not None:
        _terminate_windows_service_job(stale_metadata, spec)
...
def _stop_service(spec: ServiceSpec, *, quiet: bool = False) -> None:
    ...
    _reject_runtime_link(spec.log_path)
    metadata = _read_json(spec.metadata_path)
    process = _validated_process(spec, require_current_command=False)
    if process is None:
        if os.name == "nt" and metadata is not None:
            _terminate_windows_service_job(metadata, spec)
        spec.metadata_path.unlink(missing_ok=True)
        _retire_legacy_service(spec)
        if not quiet:
    ...
    if os.name == "nt":
        try:
            ...
            pass
        _, alive = psutil.wait_procs(targets, timeout=5.0)
        if alive and metadata is not None:
            _terminate_windows_service_job(metadata, spec)
    ...
    with FileLock(str(RUNTIME_ROOT / "launcher.lock"), timeout=30):
        if action == "start":
            ...
            for spec in (backend, frontend):
                ...
                suffix = f" (PID {process.pid}, port {spec.port})" if ready else ""
