...
def training_job_object_name(job_id: str) -> str:
    safe = "".join(
        character for character in job_id if character.isalnum() or character in "-_"
    )
    if not safe or safe != job_id:
        raise ValueError("Training Job id is unsafe for a Windows Job Object name.")
    return f"Local\\EmperorWorkbenchTraining-{safe}"
...
@dataclass(slots=True)
class PersistedWindowsJobProcessHandle:
    pid: int
    job: WindowsJob
    _terminated: bool = False

    def poll(self) -> int | None:
        if self.job.has_processes():
            return None
        return 1 if self._terminated else 0
    ...
    def wait(self, timeout: float | None = None) -> int:
        try:
            self.job.wait_empty(timeout)
        except TimeoutError as exc:
            raise subprocess.TimeoutExpired(
                ["windows-job", self.job.name], timeout or 0.0
            ) from exc
