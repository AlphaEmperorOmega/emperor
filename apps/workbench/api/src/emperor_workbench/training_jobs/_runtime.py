...
from emperor_workbench.training_jobs._lifecycle import (
    terminal_exit_code,
...
from emperor_workbench.training_jobs._status import (
...
def _model_identity(run_plan: TrainingRunPlanView) -> dict[str, str]:
    payload = RunPlanPersistenceCodec.encode(run_plan)
    return {
...
class _TrainingJobRuntime:
    ...
    def _create_job_view(
        ...
        job_id = (
            require_safe_name(command.job_id, "training job id")
        ...
        try:
            validated_log_folder = validate_log_experiment_name(
                command.run_plan.log_folder
            )
        except LogExperimentFailure as exc:
            raise TrainingJobFailure(exc.detail) from exc
        try:
            materialized = self.run_plans.materialize(
                command.run_plan,
                validated_log_folder=validated_log_folder,
            )
        except RunPlanFailure as exc:
        ...
        try:
            job = self._register_job(
                job_id=job_id,
                model=command.run_plan.model,
                materialized_run_plan=materialized_run_plan,
                selected_monitors=selected_monitors,
                planned_run_count=planned_run_count,
                validated_log_folder=validated_log_folder,
                observed_command=launch.command,
                job_root=job_root,
            ...
            self._write_event(
                job,
                {
                    "type": "job_started",
                    "status": "running",
                    **_model_identity(materialized_run_plan),
                    "preset": job.preset,
                    "presets": job.presets,
    ...
    def _build_worker_payload(
        ...
        *,
        ...
        return {
            ...
            "runPlan": RunPlanPersistenceCodec.encode(materialized_run_plan),
