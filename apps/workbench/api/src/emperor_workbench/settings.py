...
_DEFAULT_MAX_LOG_ARCHIVE_UPLOAD_SIZE = 512 * 1024 * 1024
_DEFAULT_MAX_LOG_ARCHIVE_EXTRACTED_SIZE = 2 * 1024 * 1024 * 1024
_DEFAULT_MAX_LOG_ARCHIVE_MEMBER_COUNT = 32_000
_DEFAULT_MAX_LOG_ARCHIVE_PATH_BYTES = 4 * 1024 * 1024
_DEFAULT_LOG_ARCHIVE_UPLOAD_CONCURRENCY = 1
...
def _default_workbench_root() -> Path:
    if os.name == "nt":
        local_app_data = os.environ.get("LOCALAPPDATA")
        if local_app_data:
            return Path(local_app_data) / "Emperor" / "Workbench"
        return Path.home() / "AppData" / "Local" / "Emperor" / "Workbench"

    xdg_state_home = os.environ.get("XDG_STATE_HOME")
    state_home = (
        Path(xdg_state_home) if xdg_state_home else Path.home() / ".local" / "state"
    )
    return state_home / "emperor" / "workbench"


DEFAULT_WORKBENCH_ROOT = _default_workbench_root()
DEFAULT_SNAPSHOTS_ROOT = str(DEFAULT_WORKBENCH_ROOT / "snapshots")
DEFAULT_STATE_ROOT = str(DEFAULT_WORKBENCH_ROOT / "state")
...
class WorkbenchApiSettings(BaseSettings):
    ...
    max_log_archive_member_count: int = Field(
        default=_DEFAULT_MAX_LOG_ARCHIVE_MEMBER_COUNT,
        ge=1,
    )
    max_log_archive_path_bytes: int = Field(
        default=_DEFAULT_MAX_LOG_ARCHIVE_PATH_BYTES,
        ge=1,
    )
    log_archive_upload_concurrency: int = Field(
        default=_DEFAULT_LOG_ARCHIVE_UPLOAD_CONCURRENCY,
        ge=1,
    )
    ...
    def effective_max_upload_size(self) -> int:
        ...
        return _DEFAULT_MAX_LOG_ARCHIVE_UPLOAD_SIZE
    ...
    def effective_max_log_archive_extracted_size(self) -> int:
        ...
        return _DEFAULT_MAX_LOG_ARCHIVE_EXTRACTED_SIZE
