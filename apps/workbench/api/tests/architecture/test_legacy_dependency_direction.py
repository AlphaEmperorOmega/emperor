...
from tests.support import API_ROOT, REPOSITORY_ROOT

REPO_ROOT = REPOSITORY_ROOT
SOURCE_ROOT = REPO_ROOT / "src"
CORE_PACKAGE_DIRS = ("emperor", "model_runtime", "models")
WORKBENCH_PACKAGE_DIR = API_ROOT / "src" / "emperor_workbench"
WORKBENCH_TESTS_DIR = API_ROOT / "tests"
WORKBENCH_TEST_HELPERS = WORKBENCH_TESTS_DIR / "support" / "training_jobs.py"
TRAINING_JOB_SERVICE_TEST = (
    WORKBENCH_TESTS_DIR / "unit" / "training_jobs" / "test_training_job_service.py"
)
WORKBENCH_INSPECTION_DIR = WORKBENCH_PACKAGE_DIR / "inspection"
WORKBENCH_PROJECT_ADAPTER = WORKBENCH_PACKAGE_DIR / "project_adapter"
WORKBENCH_HISTORICAL_INSPECTION_DIR = WORKBENCH_INSPECTION_DIR / "_historical"
WORKBENCH_HISTORICAL_INSPECTION = WORKBENCH_HISTORICAL_INSPECTION_DIR / "_inspection.py"
OBSOLETE_WORKBENCH_INSPECTOR_ADAPTERS = tuple(
    WORKBENCH_PACKAGE_DIR / "inspector" / filename
    for filename in (
        "__init__.py",
...
WORKBENCH_INSPECTION_SERVICE = WORKBENCH_INSPECTION_DIR / "_service.py"
WORKBENCH_RUN_PLANS_DIR = WORKBENCH_PACKAGE_DIR / "run_plans"
WORKBENCH_TRAINING_JOB_CONTRACTS = (
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "_records.py"
)
TYPED_RUN_PLAN_RUNTIME_MODULES = (
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "_runtime.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "_filesystem_store.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "_progress_projection.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "_lifecycle.py",
)
...
REMOVED_ROOT_INSPECTION_PATHS = (
    WORKBENCH_PACKAGE_DIR / "inspection_adapter.py",
    WORKBENCH_PACKAGE_DIR / "inspection_errors.py",
    WORKBENCH_PACKAGE_DIR / "inspection_serialization.py",
    WORKBENCH_PACKAGE_DIR / "inspection_worker.py",
    WORKBENCH_PACKAGE_DIR / "historical_inspection",
    WORKBENCH_PACKAGE_DIR / "services" / "inspection.py",
    WORKBENCH_PACKAGE_DIR / "api" / "v1" / "routers" / "inspection.py",
    WORKBENCH_PACKAGE_DIR / "schemas" / "_inspection.py",
)
...
REMOVED_SHALLOW_INSPECTION_PATHS = (
    WORKBENCH_PACKAGE_DIR / "services" / "models.py",
    WORKBENCH_PACKAGE_DIR / "inspector" / "config_classes.py",
    WORKBENCH_PACKAGE_DIR / "inspector" / "field_descriptions.py",
    WORKBENCH_PACKAGE_DIR / "inspector" / "overrides.py",
    WORKBENCH_PACKAGE_DIR / "inspector" / "values.py",
)
REMOVED_RUN_IMPLEMENTATION_PATHS = (
    SOURCE_ROOT / "emperor" / "experiments" / "progress.py",
    WORKBENCH_PACKAGE_DIR / "inspector" / "search.py",
    WORKBENCH_PACKAGE_DIR / "training_events.py",
)
REMOVED_TRAINING_JOB_IMPLEMENTATION_PATHS = (
    WORKBENCH_PACKAGE_DIR / "job_store.py",
    WORKBENCH_PACKAGE_DIR / "runtime" / "__init__.py",
    WORKBENCH_PACKAGE_DIR / "runtime" / "job_status.py",
    WORKBENCH_PACKAGE_DIR / "services" / "training.py",
    WORKBENCH_PACKAGE_DIR / "training_cgroups.py",
    WORKBENCH_PACKAGE_DIR / "training_contracts.py",
    WORKBENCH_PACKAGE_DIR / "training_job_lifecycle.py",
    WORKBENCH_PACKAGE_DIR / "training_job_projector.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs.py",
    WORKBENCH_PACKAGE_DIR / "training_limits.py",
    WORKBENCH_PACKAGE_DIR / "training_live_projection.py",
    WORKBENCH_PACKAGE_DIR / "training_monitor_locator.py",
    WORKBENCH_PACKAGE_DIR / "training_progress_store.py",
    WORKBENCH_PACKAGE_DIR / "training_request_commands.py",
    WORKBENCH_PACKAGE_DIR / "training_run_plans.py",
    WORKBENCH_PACKAGE_DIR / "training_run_progress.py",
    WORKBENCH_PACKAGE_DIR / "training_worker_launcher.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "plans.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "run_progress.py",
    WORKBENCH_PACKAGE_DIR / "training_jobs" / "serialization.py",
)
REMOVED_RUN_HISTORY_IMPLEMENTATION_PATHS = (
    WORKBENCH_PACKAGE_DIR / "log_experiment_mutations.py",
    WORKBENCH_PACKAGE_DIR / "log_run_artifacts.py",
    WORKBENCH_PACKAGE_DIR / "log_run_deletion.py",
    WORKBENCH_PACKAGE_DIR / "log_run_models.py",
    WORKBENCH_PACKAGE_DIR / "log_run_names.py",
    WORKBENCH_PACKAGE_DIR / "log_run_query.py",
    WORKBENCH_PACKAGE_DIR / "log_run_scanner.py",
    WORKBENCH_PACKAGE_DIR / "log_runs.py",
    WORKBENCH_PACKAGE_DIR / "monitor_data.py",
    WORKBENCH_PACKAGE_DIR / "tensorboard_reader.py",
    WORKBENCH_PACKAGE_DIR / "services" / "log_import.py",
    WORKBENCH_PACKAGE_DIR / "services" / "logs.py",
    WORKBENCH_PACKAGE_DIR / "run_history" / "checkpoint_ranking.py",
)
RUN_HISTORY_DIR = WORKBENCH_PACKAGE_DIR / "run_history"
RUN_HISTORY_RECORDS = RUN_HISTORY_DIR / "_records.py"
LOGS_HTTP_MAPPING = WORKBENCH_PACKAGE_DIR / "api" / "v1" / "run_history" / "_mapping.py"
SHARED_TENSORBOARD_DIR = WORKBENCH_PACKAGE_DIR / "tensorboard"
...
class DependencyDirectionTests(unittest.TestCase):
    ...
    def test_run_history_records_are_transport_neutral(self) -> None:
        ...
        self.assertIn("def _model_identity", mapping_source)
    ...
    def test_core_packages_do_not_import_workbench(self) -> None:
        core_roots = [SOURCE_ROOT / package_dir for package_dir in CORE_PACKAGE_DIRS]
        violations = [
            source_import.format_for_failure()
            for source_import in _absolute_imports_under(core_roots)
            if source_import.root == "emperor_workbench"
        ]
    ...
    def test_workbench_production_does_not_import_project_packages(self) -> None:
        violations = [
            source_import.format_for_failure()
            for source_import in _absolute_imports_under([WORKBENCH_PACKAGE_DIR])
    ...
    def test_historical_inspection_policy_has_one_deep_interface(self) -> None:
        ...
        inspection_test_source = "\n".join(
            path.read_text(encoding="utf-8")
            for path in sorted(
                (WORKBENCH_TESTS_DIR / "unit" / "inspection").glob("test_*.py")
            )
        )

        self.assertIn("HistoricalInspection", service_source)
        self.assertNotIn("load_checkpoint_graph_shapes", service_source)
        self.assertNotIn("def _checkpoint_overrides", service_source)
        self.assertNotIn("inspect_payload", service_source)
    ...
    def test_historical_inspection_owns_checkpoint_policy_and_interpretation(
        ...
        ranking_module = WORKBENCH_HISTORICAL_INSPECTION_DIR / "_checkpoint_ranking.py"
        ...
        self.assertTrue(
            all(not path.exists() for path in REMOVED_ROOT_INSPECTION_PATHS)
        )

    def test_domain_implementations_do_not_import_http_error_types(self) -> None:
        implementation_roots = [
            WORKBENCH_PACKAGE_DIR / "training_jobs",
            WORKBENCH_PACKAGE_DIR / "run_history",
            WORKBENCH_PACKAGE_DIR / "log_experiments",
            WORKBENCH_PACKAGE_DIR / "config_snapshots.py",
            WORKBENCH_INSPECTION_DIR,
            WORKBENCH_PACKAGE_DIR / "model_packages",
        ]
        forbidden_modules = {
            "emperor_workbench.core.errors",
    ...
    def test_workbench_run_plan_adaptation_has_one_implementation(self) -> None:
        violations: list[str] = []
        for path in _python_files([WORKBENCH_PACKAGE_DIR]):
            if (
                path.is_relative_to(WORKBENCH_RUN_PLANS_DIR)
                or path.is_relative_to(WORKBENCH_PROJECT_ADAPTER)
    ...
    def test_training_job_runtime_modules_keep_run_plans_typed(self) -> None:
        contracts_source = WORKBENCH_TRAINING_JOB_CONTRACTS.read_text(encoding="utf-8")
    ...
    def test_training_job_contracts_do_not_import_implementation_modules(self) -> None:
        violations = [
            ...
            if source_import.module.startswith("emperor_workbench.training_jobs.")
        ]
    ...
    def test_production_uses_one_config_snapshot_interface(self) -> None:
        violations = [
            ...
            for source_import in _absolute_imports_under([WORKBENCH_PACKAGE_DIR])
            if not source_import.path.is_relative_to(WORKBENCH_TESTS_DIR)
            and source_import.module == "emperor_workbench.services.config_snapshots"
        ]

        self.assertEqual([], violations)

    def test_worker_plan_tests_target_the_run_plan_interface(self) -> None:
        worker_test = (
            WORKBENCH_TESTS_DIR / "integration" / "workers" / "test_training_worker.py"
        )
        source = worker_test.read_text(encoding="utf-8")

        self.assertIn("RunPlanWorkerAcceptance.accept", source)
        self.assertNotIn("training_jobs.run_plan_adapter", source)
    ...
    def test_training_job_workflow_harness_uses_public_interface(self) -> None:
        ...
        private_runtime_accesses = [
            node.lineno
            for node in ast.walk(helper_tree)
            if isinstance(node, ast.Attribute) and node.attr == "_runtime"
        ]
        public_workflow_private_imports = [
            source_import.format_for_failure()
            for source_import in _absolute_imports(TRAINING_JOB_SERVICE_TEST)
            if source_import.module == "emperor_workbench.training_jobs._runtime"
        ]

        self.assertEqual([], private_runtime_subclasses)
        self.assertEqual([], private_runtime_accesses)
