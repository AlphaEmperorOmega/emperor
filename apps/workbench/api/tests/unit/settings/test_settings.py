...
class WorkbenchApiSettingsTests(unittest.TestCase):
    ...
    def test_settings_module_is_the_canonical_implementation(self) -> None:
        settings = importlib.import_module("emperor_workbench.settings")

        self.assertEqual(
            WorkbenchApiSettings.__module__,
    ...
    def test_old_core_config_module_is_absent(self) -> None:
    ...
    def test_default_writable_roots_are_owned_by_workbench(self) -> None:
        with isolated_settings_env():
            settings = WorkbenchApiSettings()
        ...
        if os.name == "nt":
            ...
            expected_root = (
                Path(local_app_data) / "Emperor" / "Workbench"
                if local_app_data
                else Path.home() / "AppData" / "Local" / "Emperor" / "Workbench"
            )
        else:
            ...
            expected_root = state_home / "emperor" / "workbench"

        self.assertEqual(DEFAULT_WORKBENCH_ROOT, expected_root)
        self.assertEqual(settings.snapshots_root, DEFAULT_SNAPSHOTS_ROOT)
        self.assertEqual(settings.state_root, DEFAULT_STATE_ROOT)
        self.assertEqual(
            Path(settings.snapshots_root),
            DEFAULT_WORKBENCH_ROOT / "snapshots",
        )
        self.assertEqual(Path(settings.state_root), DEFAULT_WORKBENCH_ROOT / "state")

    def test_environment_overrides_writable_roots(self) -> None:
        with isolated_settings_env(
            WORKBENCH_API_SNAPSHOTS_ROOT="custom-snapshots",
            WORKBENCH_API_STATE_ROOT="custom-state",
        ):
            settings = WorkbenchApiSettings()

        self.assertEqual(settings.snapshots_root, "custom-snapshots")
        self.assertEqual(settings.state_root, "custom-state")
