...
def fake_validate_snapshot_config(*_args: Any, **_kwargs: Any) -> None:
    return None
...
@mock.patch(
    "emperor_workbench.config_snapshots._validation.validate_config_snapshot",
    fake_validate_snapshot_config,
)
@mock.patch(
    "emperor_workbench.config_snapshots._validation.configuration_schema",
    fake_config_schema,
)
class ConfigSnapshotApiTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.addCleanup(self._close_lifespan)

    def _close_lifespan(self) -> None:
        try:
    ...
    def test_http_projection_runs_model_package_io_off_the_event_loop(self) -> None:
        service = self.app.state.workbench_container.config_snapshots
        original = service.canonical_overrides
        projection_threads: list[int] = []

        def tracked_projection(snapshot):  # type: ignore[no-untyped-def]
        ...
        with mock.patch.object(
            ...
            response = self._create(learning_rate="0.01")

        self.assertEqual(response.status_code, 200, response.text)
        self.assertTrue(projection_threads)
        self.assertTrue(
            all(thread_id != event_loop_thread for thread_id in projection_threads)
        )
    ...
    def test_update_snapshot_changes_name_and_overrides_in_place(self) -> None:
        ...
        with mock.patch(
            "emperor_workbench.config_snapshots._service._now",
