...
class TrainingCancellationCapabilityTests(unittest.TestCase):
    ...
    def test_auto_selects_windows_job_objects_on_windows(self) -> None:
        cwd = Path.cwd()
        with patch(
            "emperor_workbench.training_jobs._containment._launcher.os.name",
            "nt",
        ):
            launcher = TrainingWorkerLauncher(
                cwd=cwd,
    ...
    def test_process_group_app_construction_does_not_construct_cgroups(
        ...
        async def cancellation_capability(app) -> str:
            async with app.router.lifespan_context(app):
                training_jobs = app.state.workbench_container.training_jobs
                return training_jobs.cancellation_capability()
        ...
        with patch.object(
            ...
            capability = asyncio.run(cancellation_capability(app))
    ...
    def test_strict_capability_does_not_probe_proc_on_cgroup_v1(self) -> None:
        with TemporaryDirectory() as tmp:
            with (
                ...
                patch(
                    "emperor_workbench.training_jobs._containment._cgroup_v2."
