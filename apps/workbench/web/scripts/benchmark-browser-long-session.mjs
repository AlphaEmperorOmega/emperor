...
function installPageInstrumentation(apiBaseUrl) {
  ...
  console.error = (...values) => {
    state.errors.push(values.map(errorText).join(" "));
  ...
  state.clickWorkspace = (name) => {
    ...
    const expected = normalize(name).toLowerCase();
    const link = [...(navigation?.querySelectorAll("a[href]") ?? [])].find(
      (candidate) =>
  ...
  state.clickListboxTrigger = (label) => {
  ...
  try {
    diagnostics = await evaluate(
      ...
      `(() => ({
        ...
        pageErrors: window.__EMPEROR_BROWSER_PERFORMANCE__?.errors ?? [],
        readyState: document.readyState,
        selectors: {
          modelTrigger: Boolean(
            document.querySelector(
              "button[aria-haspopup=listbox][aria-label='model']",
            ),
          ),
          reactFlow: Boolean(document.querySelector(".react-flow")),
    ...
    // Preserve the original timeout when the execution context is unavailable.
  }
  throw new Error(
    `Timed out waiting for ${label}: ${JSON.stringify(diagnostics)}.`,
  );
}
...
async function clickWorkspace(client, workspace) {
  ...
  await waitForExpression(
    client,
    `[...document.querySelectorAll("nav[aria-label='Workspace'] a[href]")].some(
      (link) => link.textContent.trim() === ${JSON.stringify(workspace)} &&
...
async function selectDropdown(client, label, option, contains = false) {
  ...
  const selected = await pageMethod(client, "clickOption", [option, contains]);
  if (selected?.ok) {
    return;
  }
  await requirePageMethod(
  ...
  await waitForExpression(
    client,
    `window.__EMPEROR_BROWSER_PERFORMANCE__.options().some((candidate) => {
      const actual = candidate.toLowerCase();
      const expected = ${JSON.stringify(option.toLowerCase())};
      return ${contains ? "actual.includes(expected)" : "actual === expected"};
    })`,
    `${label} filtered option ${option}`,
  );
  await requirePageMethod(
    client,
    "clickOption",
...
async function measureNavigation(client, frontendUrl) {
  ...
  await waitForExpression(
    client,
    "document.querySelector('nav[aria-label=Workspace]') && " +
      "document.querySelector(\"button[aria-haspopup=listbox][aria-label='model']\") && " +
...
async function runWorkspaceCycle(client, preset) {
  return measureOperation(client, `workspace_cycle_${preset}`, async () => {
    ...
    await waitForExpression(
      client,
      `document.querySelector("button[aria-haspopup=listbox][aria-label='preset']")
...
async function selectBrowserPerformanceLogs(client) {
  ...
  await waitForExpression(
    client,
    `[...document.querySelectorAll("button[aria-haspopup='listbox']")].some((candidate) =>
  ...
  const opened = await evaluate(
    client,
    `(() => {
      const control = [...document.querySelectorAll("button[aria-haspopup='listbox']")]
        .find((candidate) =>
          candidate.getAttribute("aria-label")?.toLowerCase().startsWith("experiments"),
        );
      if (!control) {
...
async function completeTrainingJob(client) {
  return measureOperation(client, "training_job_completion", async () => {
    ...
    await waitForExpression(
      client,
      `document.querySelector(
        "#training-workspace [data-training-job-status='completed']"
      ) !== null`,
...
async function prepareNeuronGraph(client) {
  ...
  await waitForExpression(
    client,
    "document.querySelector(\"button[aria-haspopup=listbox][aria-label='model']\") && " +
      "!document.querySelector(\"button[aria-haspopup=listbox][aria-label='model']\").disabled",
    "neuron model selection",
  );
  const modelName = await evaluate(
    client,
    "document.querySelector(\"button[aria-haspopup=listbox][aria-label='model']\").textContent.trim()",
  );
  if (!modelName) {
...
async function runBenchmark(options) {
  ...
  delete backendEnvironment.PYTHONPATH;
  delete backendEnvironment.PYTHONSAFEPATH;
