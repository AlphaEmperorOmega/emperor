...
const BACKEND_CONTACT_EXCEPTIONS: BackendContactException[] = [
  ...
  {
    relativePath: "src/lib/api/model-catalog.ts",
    reason: "The lightweight Model Catalog API module owns the startup model-list route.",
  },
  {
    relativePath: "src/lib/api/models.ts",
    reason: "Deferred Model Package metadata API module owns presets, datasets, monitors, schema, and search-space routes.",
  },
  {
...
const DELETED_PUBLIC_API_FACADE_PATH = "src/lib/api.ts";
  ...
  function visit(node: ts.Node) {
    if (
      ...
      addViolation(
        ...
        sourceFile,
        node,
        "import from the deleted broad @/lib/api facade",
      );
    }
    if (
      ...
      node.arguments[0].text === "@/lib/api"
    ) {
      addViolation(
        violations,
        sourceFile,
        node,
        "dynamic import from the deleted broad @/lib/api facade",
      );
    }
...
describe("frontend API boundary", () => {
  it("keeps production backend contact restricted to approved API modules", () => {
    ...
    expect(productionFilePaths).not.toContain(DELETED_PUBLIC_API_FACADE_PATH);
