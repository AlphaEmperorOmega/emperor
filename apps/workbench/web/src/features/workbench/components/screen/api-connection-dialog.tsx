...
export function ApiConnectionDialog({ onClose }: { onClose: () => void }) {
  ...
  const [apiDraft, setApiDraft] = useState<{
    ...
    inputValue: string;
    error: string | null;
  }>(() => ({ apiBaseUrl, inputValue: apiBaseUrl, error: null }));
  const activeApiDraft =
    apiDraft.apiBaseUrl === apiBaseUrl
      ? apiDraft
      : { apiBaseUrl, inputValue: apiBaseUrl, error: null };
  const inputValue = activeApiDraft.inputValue;
  const error = activeApiDraft.error;
