...
describe("TrainingRunPlanCard", () => {
  ...
  it("exposes a stable completed Training Job status contract", () => {
    const runPlan = plan([
      run({ index: 1, status: "Completed", currentEpoch: 30 }),
    ]);
    ...
    renderCard({
      plan: runPlan,
      job: job(runPlan, { status: "completed", exitCode: 0 }),
    });

    expect(
      screen
        .getByText("Training Job Results")
        .closest("[data-training-job-status]"),
    ).toHaveAttribute("data-training-job-status", "completed");
  });
