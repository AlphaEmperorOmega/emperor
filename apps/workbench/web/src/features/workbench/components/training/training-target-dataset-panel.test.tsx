...
describe("Training setup Adapter", () => {
  it("renders the grouped setup ownership and switches variant projections", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "training model type" }),
    ).toHaveTextContent("Linears");
    expect(
      screen.getByRole("button", { name: "Experiment task" }),
    ).toHaveTextContent("Image Classification");
    expect(
      screen.getByRole("button", { name: /^Presets 1 \/ 1 selected$/ }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Config snapshots" }),
