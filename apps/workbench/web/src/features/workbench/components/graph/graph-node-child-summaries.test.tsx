...
describe("GraphNodeChildSummaries", () => {
  it("uses the W/b indicator group as the only activity popup trigger", async () => {
    ...
    expect(await screen.findByRole("tooltip")).toHaveTextContent("Weights");

    fireEvent.mouseLeave(indicators);
    expect(screen.queryByRole("tooltip")).not.toBeInTheDocument();

    fireEvent.focus(indicators);
    expect(await screen.findByRole("tooltip")).toHaveTextContent("Bias");
