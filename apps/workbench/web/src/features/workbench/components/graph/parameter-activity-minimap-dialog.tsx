...
function ParameterActivityMinimapDialogSession({
  model,
  graph,
  activityByNodePath,
  source,
...
function ParameterActivityMinimapDialogContent(
  ...
  const model = useMemo(
    () =>
      deriveParameterActivityMinimapModel({
        graph: props.graph,
        activityByNodePath: props.activityByNodePath,
      }),
    [props.activityByNodePath, props.graph],
  );
  const sessionKey = JSON.stringify([
    props.graph.nodes.map((node) => node.id),
    [...model.relevantNodeIds].sort(),
  ]);

  return (
    <ParameterActivityMinimapDialogSession
      key={sessionKey}
      {...props}
      model={model}
    />
