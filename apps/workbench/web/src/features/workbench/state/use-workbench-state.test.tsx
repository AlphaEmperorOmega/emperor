...
vi.mock("@/lib/api/client", async (importOriginal) => ({
  ...
  isUnauthorizedApiError: mocks.isUnauthorizedApiError,
}));
...
vi.mock("@/lib/api/config-snapshots", async (importOriginal) => ({
  ...
  fetchConfigSnapshots: mocks.fetchConfigSnapshots,
  fetchConfigSnapshotLibrary: mocks.fetchConfigSnapshotLibrary,
  createConfigSnapshot: mocks.createConfigSnapshot,
  renameConfigSnapshot: mocks.renameConfigSnapshot,
  updateConfigSnapshot: mocks.updateConfigSnapshot,
  deleteConfigSnapshot: mocks.deleteConfigSnapshot,
}));
vi.mock("@/lib/api/log-query-client", () => ({
  fetchLogRuns: async (
    ...args: Parameters<typeof import("@/lib/api/logs").fetchLogRuns>
  ) => {
...
vi.mock("@/lib/api/monitor-data", async (importOriginal) => ({
  ...
  fetchMonitorParameterStatus: mocks.fetchMonitorParameterStatus,
  fetchLogParameterStatus: mocks.fetchLogParameterStatus,
}));
...
import { getPersistedTargetSelectionSnapshot } from "@/features/workbench/state/target/target-selection-storage";
import type { GraphNode, InspectResponse } from "@/lib/api/inspection";
import type { LogParameterStatusResponse } from "@/lib/api/monitor-data";
...
function TrainingSnapshotsReadyProbe({ testId }: { testId: string }) {
  const snapshotCount = useTrainingDraft().setup.variants.snapshots.length;
  return <output data-testid={testId}>{snapshotCount}</output>;
}

function fetchReadyConfigSnapshots(identity: ModelIdentity) {
  return Promise.resolve({
    ...identity,
    snapshots: [
      {
        id: "render-isolation-ready",
        ...identity,
        preset: "baseline",
        name: "Render isolation ready",
        overrides: {},
...
describe("useWorkbenchState", () => {
  ...
  it("does not rerender Training consumers for a Model-only override", async () => {
    ...
    mocks.fetchConfigSnapshots.mockImplementation(fetchReadyConfigSnapshots);
      ...
      <QueryClientProvider client={client}>
        <WorkbenchProviders activeWorkspace="training">
          ...
          >
            ...
            <TrainingSnapshotsReadyProbe testId="training-snapshots-ready" />
          </TrainingExecutionProvider>
        </WorkbenchProviders>
      </QueryClientProvider>,
    );
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("training-snapshots-ready")).toHaveTextContent(
        "1",
      );
    });
  ...
  it("does not rerender Model consumers for a Training-only override", async () => {
    ...
    const ModelTargetProbe = memo(function ModelTargetProbe() {
      const { browser, runtimeDefaults, status } = useModelPackageInspection();
      modelRenderCount += 1;
      return (
        <output
          data-testid="model-target-probe"
          data-ready={
            browser.selectedPreset &&
            browser.selectedDatasets.length > 0 &&
            status.schema.isReady &&
            !status.monitors.isLoading
              ? "true"
              : "false"
          }
        >
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("model-target-probe")).toHaveAttribute(
        "data-ready",
        "true",
      );
      expect(
  ...
  it("profiles a Model-only override without committing unrelated target consumers", async () => {
    ...
    function SnapshotsProbe() {
      ...
      renders.snapshots += 1;
      return <output data-testid="profile-snapshots">{records.allCount}</output>;
    }
        ...
        <WorkbenchProviders activeWorkspace="training">
          ...
          >
            ...
            >
              ...
              <TrainingSnapshotsReadyProbe testId="profile-training-snapshots" />
              <TrainingProbe />
            </TrainingExecutionProvider>
          </Profiler>
        </WorkbenchProviders>
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("profile-snapshots")).toHaveTextContent("1");
      expect(
        screen.getByTestId("profile-training-snapshots"),
      ).toHaveTextContent(
        "1",
      );
    });
  ...
  it("selects the first model in a new type through the model reset cascade", async () => {
    ...
    expect(getPersistedTargetSelectionSnapshot()).toEqual({
      selectedModelType: "experts",
      selectedModel: "linear",
      selectedPreset: "expert-baseline",
  ...
  it("uses footer-style icon labels in the sidebar target selectors", async () => {
    ...
    await screen.findByRole("button", { name: /^model type$/i });
  ...
  it("switches the sidebar model dropdown without an update loop", async () => {
    ...
    const modelControl = await screen.findByRole("button", { name: /^model$/i });
  ...
  it("filters the sidebar model dropdown by model type", async () => {
    ...
    const modelTypeControl = await screen.findByRole("button", {
      name: /^model type$/i,
    });
    const modelControl = await screen.findByRole("button", { name: /^model$/i });
  ...
  it("renders edit and duplicate actions under the active snapshot selector", async () => {
    ...
    const snapshotSelect = await screen.findByRole("button", {
      name: /^snapshot$/i,
    });
  ...
  it("does not render snapshot actions in experiment target mode", async () => {
    ...
    expect(await screen.findByRole("button", { name: "Experiment" }))
      .toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Dataset" })).toBeDisabled();
    expect(screen.getByRole("button", { name: "Preset" })).toBeDisabled();
    expect(screen.queryByRole("button", { name: /^experiment run$/i }))
      .not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Create Snapshot" }))
      .not.toBeInTheDocument();
    expect(screen.queryByRole("button", { name: "Edit" })).not.toBeInTheDocument();
  ...
  it("switches the training model dropdown without an update loop", async () => {
    ...
    const modelControl = await within(panel).findByRole("button", {
  ...
  it("filters the training model dropdown by model type", async () => {
    ...
    const modelTypeControl = await within(panel).findByRole("button", {
  ...
  it("allows training while a historical experiment run is selected", async () => {
    ...
    const datasetControl = screen.getByRole("button", { name: "Dataset" });
    ...
    const presetControl = screen.getByRole("button", { name: "Preset" });
