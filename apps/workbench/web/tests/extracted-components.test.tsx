...
describe("FullPageError", () => {
  ...
  it("renders a diagnostic reference separately from safe error copy", () => {
    render(<FullPageError reference="digest-123" onRetry={() => {}} />);
...
describe("WorkbenchWorkspaceNav", () => {
  it("marks the active workspace and emits workspace changes", async () => {
    ...
    render(
      <WorkbenchWorkspaceNav
        ...
        hrefForWorkspace={(workspace) => `/?theme=night&workspace=${workspace}`}
        onChange={onChange}
      />,
    );
    ...
    expect(workspaceLinks.map((link) => link.textContent?.trim())).toEqual([
      "Model",
      "Training",
      "Logs",
    ]);
    ...
    expect(modelLink).toHaveAttribute("aria-current", "page");
    expect(modelLink).toHaveAttribute(
      "href",
      "/?theme=night&workspace=model",
    );
    expect(modelLink.className).not.toContain("after:");
    expect(trainingLink).not.toHaveAttribute("aria-current");
    expect(logsLink).not.toHaveAttribute("aria-current");
    expect(modelLink).not.toHaveAttribute("aria-pressed");
    expect(trainingLink).not.toHaveAttribute("aria-pressed");
    expect(logsLink).not.toHaveAttribute("aria-pressed");
  ...
  it.each([
    ...
    render(
      <WorkbenchWorkspaceNav
        activeWorkspace="model"
        hrefForWorkspace={(workspace) => `/?workspace=${workspace}`}
        onChange={vi.fn()}
    ...
    expect(within(trainingLink).getByText(label)).toBeInTheDocument();
  });
});
