...
function renderTrainingInterfaceHarness({
  ...
  let trainingActivated = activeWorkspace === "training";
  const renderTree = (workspace: WorkbenchWorkspace) => {
    trainingActivated ||= workspace === "training";
    return (
      <QueryClientProvider client={queryClient}>
        <WorkbenchProviders activeWorkspace={workspace}>
          <TrainingExecutionProvider
            activated={trainingActivated}
...
describe("WorkbenchApp Training And Preview", () => {
  ...
  it("mounts Training as its own workspace", async () => {
    ...
    await user.click(screen.getByRole("link", { name: /^logs$/i }));
  ...
  it("Training workspace shows the three-zone setup, run list, and status layout", async () => {
    ...
    const modelSelector = within(setupSidebar).getByRole("button", {
      name: /^training model$/i,
    });
    const presetSelector = within(setupSidebar).getByRole("button", {
      name: /^presets\s+1\s*\/\s*2 selected$/i,
    });
    const datasetSelector = within(setupSidebar).getByRole("button", {
      name: /^training datasets\s+1\s*\/\s*2 selected$/i,
    });
    const monitorSelector = within(setupSidebar).getByRole("button", {
    ...
    const logFolderSelector = within(setupSidebar).getByRole("button", {
    ...
    const logFolderDropdown = within(setupSidebar).getByRole("button", {
      name: "Log experiment folder",
    });
    ...
    expect(
      within(details).queryByRole("button", { name: /^training preset$/i }),
    ).not.toBeInTheDocument();
    expect(modelSelector).toHaveTextContent("linear");
    expect(presetSelector).toHaveTextContent("baseline");
    expect(
      within(details).getAllByRole("button", { name: /^presets\b/i }),
    ).toHaveLength(1);
    ...
    await waitFor(() => {
      expect(
        within(setupSidebar).getByRole("button", {
    ...
    await waitFor(() => {
      const clearedMonitorSelector = within(setupSidebar).getByRole("button", {
        name: /^training monitors\s+0\s*\/\s*2 selected$/i,
      });
    ...
    expect(
      within(setupSidebar).queryByRole("combobox", {
        name: /search config fields/i,
      }),
    ).not.toBeInTheDocument();
    expect(
    ...
    await user.keyboard("{Escape}");
    await waitFor(() => {
      const selectedMonitorSelector = within(setupSidebar).getByRole("button", {
  ...
  it("filters training multiselect options and selects matching results", async () => {
    ...
    await user.type(
      within(details).getByRole("combobox", { name: /^search presets$/i }),
      "recurrent",
    );
    ...
    await user.type(
      within(details).getByRole("combobox", { name: /^search training datasets$/i }),
      "cifar",
    );
  ...
  it("prunes datasets when the Training Experiment Task changes", async () => {
    ...
    const experimentTask = within(details).getByRole("button", {
  ...
  it("checks the started experiment when switching to logs", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: /^experiments\b/i }));
