...
class TestMultiHeadAttention(TestAttention):
    ...
    def test__build_shared_projection_models(self):
        c = copy.deepcopy(self.cfg)
        # This exists here to ensure because of `register_parameter` in the
        # this method is allready called when `MultiHeadAttention` is initialized
        c.multi_head_attention_model_config.embedding_dim = 32
    ...
    def test__build_separate_projection_models(self):
        c = copy.deepcopy(self.cfg)
        c.multi_head_attention_model_config.embedding_dim = 32
        c.multi_head_attention_model_config.key_dim = 64
        c.multi_head_attention_model_config.value_dim = 32
    ...
    def test__build_projection_models__same_qkv_dim(self):
        c = copy.deepcopy(self.cfg)
        c.multi_head_attention_model_config.embedding_dim = 32
        c.multi_head_attention_model_config.key_dim = 32
        c.multi_head_attention_model_config.value_dim = 32
    ...
    def test__build_projection_models__different__qkv_dim(self):
        c = copy.deepcopy(self.cfg)
        c.multi_head_attention_model_config.embedding_dim = 32
        c.multi_head_attention_model_config.key_dim = 64
        c.multi_head_attention_model_config.value_dim = 32
...
class TestAttentionUtils(TestAttention):
    def test__transpose_shared_qkv__same_kv(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__transpose_shared_qkv__different_kv(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        validator = AttentionValidator(config)
        m = AttentionUtils(config, validator)
    ...
    def test__is_tensor_batched__using_non_batched_tensor(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        validator = AttentionValidator(config)
        m = AttentionUtils(config, validator)
        ...
        tensor = torch.randn(sequence_length, embedding_dim)
        output = m._AttentionUtils__is_tensor_batched(tensor)
        self.assertFalse(output)

    def test__is_tensor_batched__using_batched_tensor(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        validator = AttentionValidator(config)
        m = AttentionUtils(config, validator)
    ...
    def test__maybe_transpose_qkv__batch_first__False(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__maybe_transpose_qkv__batch_first__True(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__maybe_transpose_qkv__batch_first__True__same_kv(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        config.batch_first_flag = True
        validator = AttentionValidator(config)
    ...
    def test__maybe_transpose_qkv__batch_first__True__different__qkv(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        config.batch_first_flag = True
        validator = AttentionValidator(config)
...
class TestAttentionValidator(TestAttention):
    ...
    def test__check_query_key_value_dimensions__batched_input_flag__True__incorrect_input_dim(
        self,
    ):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__check_query_key_value_dimensions__batched_input_flag__False__correct_input_dim(
        self,
    ):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__check_query_key_value_dimensions__batched_input_flag__True__correct_input_dim(
        self,
    ):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__check_key_padding_mask_dimensions_None(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
    ...
    def test__check_key_padding_mask_dimensions__incorrect_input_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
    ...
    def test__check_key_padding_mask_dimensions__batched_input_flag__True(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_key_padding_mask_dimensions__batched_input_flag__False(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = False
    ...
    def test__check_attention_mask__None(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_attention_mask__assert_one_dim__incorrect_input_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_attention_mask__assert_two_dim__incorrect_input_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_attention_mask__assert_two_dim__correct_input_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_attention_mask__assert_three_dim__incorrect_input_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__check_attention_mask__assert_three_dim__correct_input_shape(
        self,
    ):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
    ...
    def test__check_attention_mask__assert_four_dim__not_supported(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__resolve_attention_mask_shape__two_dim_shape(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__ensure_attention_mask_if_causal__causal_attention_mask_flag__False(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.causal_attention_mask_flag = True
    ...
    def test__multi_head_attention_input_shapes__batched_input_flag__True(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = True
    ...
    def test__multi_head_attention_input_shapes__batched_input_flag__False(self):
        c = copy.deepcopy(self.cfg)
        config = c.multi_head_attention_model_config
        m = AttentionValidator(config)
        m.batched_input_flag = False
