...
class TestMatrixChoiceMixture(unittest.TestCase):
    ...
    def test__init_with_default_config(self):
        m = MatrixChoiceMixture(self.cfg)
        input = self.mixture_cfg.input_dim
        depth = self.mixture_cfg.depth_dim
        output = self.mixture_cfg.output_dim

        self.assertIsInstance(m.weight_bank, Parameter)
        self.assertEqual(list(m.weight_bank.shape), [depth, input, output])
        self.assertIsInstance(m.bias_bank, Parameter)
        self.assertEqual(list(m.bias_bank.shape), [depth, output])

    def test__init_parameter_banks(self):
        sz = lambda x: torch.Size(x)
        m = MatrixChoiceMixture(self.cfg)
        weight_bank, bias_bank = m._MatrixChoiceMixture__init_parameter_banks()
        input = self.mixture_cfg.input_dim
        depth = self.mixture_cfg.depth_dim
        output = self.mixture_cfg.output_dim

        self.assertIsInstance(weight_bank, Parameter)
        self.assertEqual(weight_bank.shape, sz([depth, input, output]))

        self.assertIsInstance(bias_bank, Parameter)
        self.assertEqual(bias_bank.shape, sz([depth, output]))
    ...
    def test__select_parameters_top_1(self):
        ...
        overrides = MixtureConfig(top_k=1)
        m = MatrixChoiceMixture(self.cfg, overrides)

        weight_indexes = torch.randint(0, depth, (batch_size,))
        bias_indexes = torch.randint(0, depth, (batch_size,))
    ...
    def test__select_parameters_top_k(self):
        ...
        weight_indexes = torch.randint(0, depth, (batch_size, top_k))
        bias_indexes = torch.randint(0, depth, (batch_size, top_k))

        selected_weights, selected_biases = m._select_parameters(
            weight_indexes, bias_indexes
        )

        self.assertEqual(selected_weights.shape, sz([batch_size, top_k, input, output]))
        self.assertEqual(selected_biases.shape, sz([batch_size, top_k, output]))
    ...
    def test__compute_parameter_mixture_top_1(self):
        ...
        overrides = MixtureConfig(top_k=top_k, weighted_parameters_flag=True)
        m = MatrixChoiceMixture(self.cfg, overrides)
        ...
        weight_probs_test = weight_probs.reshape([-1, top_k, 1])
        bias_probs_test = bias_probs.reshape([-1, top_k])
    ...
    def test__compute_parameter_mixture_top_1_weighted_parameters_false(self):
        ...
        overrides = MixtureConfig(top_k=top_k, weighted_parameters_flag=False)
        m = MatrixChoiceMixture(self.cfg, overrides)
    ...
    def test__compute_parameter_mixture_top_k(self):
        ...
        weight_indexes = torch.randint(0, depth, (batch_size, top_k))
        bias_indexes = torch.randint(0, depth, (batch_size, top_k))
        weight_probs = F.softmax(torch.randn(batch_size, top_k), dim=-1)
        bias_probs = F.softmax(torch.randn(batch_size, top_k), dim=-1)
        ...
        weight_probs_test = weight_probs.reshape([-1, top_k, 1, 1])
        bias_probs_test = bias_probs.reshape([-1, top_k, 1])

        weighted_weights = selected_weights * weight_probs_test
        weighted_biases = selected_biases * bias_probs_test

        expected_weight_mixture = weighted_weights.sum(dim=1)
        expected_bias_mixture = weighted_biases.sum(dim=1)
    ...
    def test__compute_parameter_mixture_top_k_weighted_parameters_false(self):
        ...
        expected_weight_mixture = selected_weights.sum(dim=1)
        expected_bias_mixture = selected_biases.sum(dim=1)
    ...
    def test__compute_parameter_mixture_full(self):
        ...
        weight_probs = F.softmax(torch.randn(batch_size, router_output_dim), dim=-1)
        bias_probs = F.softmax(torch.randn(batch_size, router_output_dim), dim=-1)
        ...
        weight_probs_test = weight_probs.reshape([-1, router_output_dim, 1, 1])
        bias_probs_test = bias_probs.reshape([-1, router_output_dim, 1])
    ...
    def test__compute_parameter_mixture_full_weighted_parameters_false(self):
        ...
        weight_logits = torch.randn(batch_size, router_output_dim)
        weight_mask = weight_logits > 0
        weight_logits = weight_logits.masked_fill(weight_mask, float("-inf"))
        weight_probs = F.softmax(weight_logits, dim=-1)

        bias_logits = torch.randn(batch_size, router_output_dim)
        bias_mask = bias_logits > 0
        bias_logits = bias_logits.masked_fill(bias_mask, float("-inf"))
        bias_probs = F.softmax(bias_logits, dim=-1)
        ...
        weight_probs_shape = [-1, router_output_dim, 1, 1]
        weight_probs_test = (weight_probs > 0).int().reshape(weight_probs_shape)

        bias_probs_shape = [-1, router_output_dim, 1]
        bias_probs_test = (bias_probs > 0).int().reshape(bias_probs_shape)
...
class TestGeneratorChoiceMixture(unittest.TestCase):
    ...
    #
    #     overrides = MixtureConfig(
    #         top_k=top_k, weighted_parameters_flag=True, bias_parameters_flag=True
    #     )
    #     m = GeneratorChoiceMixture(self.cfg, overrides)
    #     print(m.bias_parameters_flag)
    #
    #     input_batch = torch.randn(batch_size, input)
    #     weight_indexes = torch.randint(0, depth, (batch_size,))
    #     bias_indexes = torch.randint(0, depth, (batch_size,))
    #     weight_probs = F.sigmoid(torch.randn((batch_size,)))
    #     bias_probs = F.sigmoid(torch.randn((batch_size,)))
    ...
    #     top_k = self.mixture_cfg.top_k
    #
    #     overrides = MixtureConfig(weighted_parameters_flag=True)
    #     m = GeneratorChoiceMixture(self.cfg, overrides)
    #
    #     input_batch = torch.randn(batch_size, input)
    #     weight_indexes = torch.randint(0, depth, (batch_size, top_k))
    #     bias_indexes = torch.randint(0, depth, (batch_size, top_k))
    #     weight_probs = F.sigmoid(torch.randn((batch_size, top_k)))
    #     bias_probs = F.sigmoid(torch.randn((batch_size, top_k)))
    #
    #     (
    #         selected_input_params,
    #         selected_output_params,
    ...
    #
    #     input_batch = torch.randn(batch_size, input)
    #     weight_indexes = torch.randint(0, depth, (batch_size, router_output_dim))
    #     bias_indexes = torch.randint(0, depth, (batch_size, router_output_dim))
    #     weight_probs = F.sigmoid(torch.randn((batch_size, router_output_dim)))
    #     bias_probs = F.sigmoid(torch.randn((batch_size, router_output_dim)))
    #
    #     (
    #         selected_input_params,
    #         selected_output_params,
