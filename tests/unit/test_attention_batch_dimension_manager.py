...
class TestNormalize(TestBatchDimensionManager):
    def test_shared_qkv_input_tensors_preserve_identity(self):
        ...
        output_q, output_k, output_v, _, _, runtime_shape = self.model.normalize(
            query,
            query,
            query,
        )
        ...
        self.assertIs(output_q, output_k)
        self.assertIs(output_k, output_v)
        torch.testing.assert_close(output_q, query.transpose(0, 1))
    ...
    def test_shared_key_value_input_tensors_preserve_identity(self):
        ...
        self.assertIs(output_k, output_v)
        torch.testing.assert_close(output_q, query.transpose(0, 1))
        torch.testing.assert_close(output_k, key.transpose(0, 1))
        self.assertTrue(runtime_shape.input_was_batch_first)
