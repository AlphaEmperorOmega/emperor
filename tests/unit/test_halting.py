...
class TestHalting(unittest.TestCase):
    ...
    def test_handcrafted_logits_match_stick_breaking_equations(self):
        ...
        state = model._StickBreaking__update_state(initial_state, log_gates_2, hidden_2)
        ...
        self.assertTrue(
            torch.equal(initial_state.halt_mask, expected_initial_halt_mask)
        )
...
class TestSoftHalting(unittest.TestCase):
    ...
    def test_config_registry_owner_builds_configured_layer_stack(self):
        cfg = self.preset(input_dim=6, gate_num_layers=2)

        self.assertIs(cfg._registry_owner(), SoftHalting)
        model = cfg.build()

        self.assertIsInstance(model, SoftHalting)
        self.assertEqual(model.input_dim, 6)
        self.assertEqual(model.threshold, cfg.threshold)
        self.assertEqual(model.hidden_state_mode, cfg.hidden_state_mode)
        self.assertIs(model.halting_gate_config, cfg.halting_gate_config)
        self.assertIsInstance(model._gate, LayerStack)
        self.assertEqual(len(model._gate), 2)
        self.assertEqual(model._gate[0].input_dim, 6)
        self.assertEqual(model._gate[-1].output_dim, 2)

    def test_overrides_take_precedence_and_keep_base_gate_config(self):
        base = self.preset(input_dim=4, threshold=0.99)
    ...
    def test_validation_matches_public_halting_contract(self):
        invalid_cases = [
            ("missing_input_dim", "input_dim", None, ValueError),
            ("invalid_input_dim", "input_dim", 0, ValueError),
            ("invalid_threshold", "threshold", 0.0, ValueError),
            ("missing_hidden_mode", "hidden_state_mode", None, ValueError),
            ("wrong_gate_type", "halting_gate_config", object(), TypeError),
        ]
        for name, field_name, value, exception_type in invalid_cases:
            with self.subTest(name=name):
                cfg = self.preset()
                setattr(cfg, field_name, value)

                with self.assertRaisesRegex(exception_type, field_name):
                    SoftHalting(cfg)
    ...
    def test_gate_logits_support_rank_two_and_three_inputs(self):
        model = self.preset(input_dim=3).build()
        model.eval()

        rank_two = model._SoftHalting__compute_gate_logits(torch.ones(5, 3))
        rank_three = model._SoftHalting__compute_gate_logits(torch.ones(2, 4, 3))

        self.assertEqual(rank_two.shape, (5, 2))
        self.assertEqual(rank_three.shape, (2, 4, 2))
        torch.testing.assert_close(
            rank_two.exp().sum(dim=-1),
            torch.ones(5),
        )
        torch.testing.assert_close(
            rank_three.exp().sum(dim=-1),
            torch.ones(2, 4),
        )

    def test_state_updates_follow_soft_stick_breaking_math(self):
        model = self.preset(input_dim=2).build()
        model.eval()
        first_hidden = torch.tensor([[[1.0, 2.0], [3.0, 4.0]]])
        second_hidden = torch.tensor([[[2.0, 4.0], [6.0, 8.0]]])
        third_hidden = torch.tensor([[[4.0, 8.0], [12.0, 16.0]]])

        state, output = model.update_halting_state(None, first_hidden)
        self.assertIsInstance(state, SoftHaltingState)
        self.assertEqual(state.step_count, 0)
        torch.testing.assert_close(
            state.accumulated_hidden, torch.zeros_like(first_hidden)
        )
        torch.testing.assert_close(state.continuation_probability, torch.ones(1, 2))
        torch.testing.assert_close(output, first_hidden)

        state, output = model.update_halting_state(state, second_hidden)
        self.assertEqual(state.step_count, 1)
        torch.testing.assert_close(
            state.log_continuation,
            torch.full((1, 2), torch.log(torch.tensor(0.5))),
        )
        torch.testing.assert_close(
            state.continuation_probability, torch.full((1, 2), 0.5)
        )
        torch.testing.assert_close(state.accumulated_hidden, second_hidden * 0.5)
        torch.testing.assert_close(state.accumulated_ponder_cost, torch.zeros(1, 2))
        torch.testing.assert_close(output, second_hidden)

        state, _output = model.update_halting_state(state, third_hidden)
        self.assertEqual(state.step_count, 2)
        torch.testing.assert_close(
            state.continuation_probability, torch.full((1, 2), 0.25)
        )
        torch.testing.assert_close(
            state.accumulated_hidden,
            second_hidden * 0.5 + third_hidden * 0.25,
        )
        torch.testing.assert_close(
            state.accumulated_ponder_cost, torch.full((1, 2), 0.25)
        )

    def test_accumulated_mode_and_pad_mask_state(self):
        model = self.preset(
            input_dim=2,
            hidden_state_mode=HaltingHiddenStateModeOptions.ACCUMULATED,
        ).build()
        ...
        pad_mask = torch.tensor([[1.0, 0.0]])

        state, initial_output = model.update_halting_state(
            None,
            hidden,
            pad_mask,
        )
        state, updated_output = model.update_halting_state(
            state,
            hidden,
            pad_mask,
        )

        torch.testing.assert_close(initial_output, torch.zeros_like(hidden))
        torch.testing.assert_close(updated_output, hidden * 0.5)
        torch.testing.assert_close(
            state.continuation_probability,
            torch.tensor([[0.5, 0.0]]),
        )

    def test_compute_output_matches_blending_and_ponder_math(self):
        model = self.preset(input_dim=2).build()
        model.eval()
        hidden = torch.tensor([[[2.0, 4.0], [6.0, 8.0]]])
        state, _ = model.update_halting_state(None, hidden)

        initial_output, initial_loss = model.compute_output(
            state,
            hidden,
            torch.zeros_like(hidden),
            torch.ones(1, 2),
        )
        torch.testing.assert_close(initial_output, hidden)
        torch.testing.assert_close(initial_loss, hidden.new_zeros(()))

        state, _ = model.update_halting_state(state, hidden)
        state, _ = model.update_halting_state(state, hidden * 2)
        current_hidden = hidden * 3
        output, act_loss = model.compute_output(
            state,
            current_hidden,
            torch.zeros_like(hidden),
            torch.ones(1, 2),
        ...
        expected = state.accumulated_hidden + 0.25 * current_hidden
        torch.testing.assert_close(output, expected)
        torch.testing.assert_close(act_loss, torch.tensor(0.75))

    def test_device_and_dtype_are_preserved(self):
        ...
        for device in devices:
            for dtype in (torch.float32, torch.float64):
                with self.subTest(device=device, dtype=dtype):
                    ...
                    model.eval()
                    hidden = torch.ones(1, 2, 2, device=device, dtype=dtype)
                    pad_mask = torch.ones(1, 2, device=device, dtype=dtype)

                    state, _ = model.update_halting_state(None, hidden, pad_mask)
                    output, loss = model.compute_output(
                        state,
                        hidden,
                        torch.zeros_like(hidden),
                        pad_mask,
                    )
                    state, _ = model.update_halting_state(state, hidden, pad_mask)
                    ...
                    for state_tensor in (
                        ...
                        self.assertEqual(state_tensor.device, device)
                        self.assertEqual(state_tensor.dtype, dtype)

    def test_state_dict_round_trip_preserves_gate_outputs(self):
        cfg = self.preset(input_dim=3, gate_num_layers=2)
        model = cfg.build()
        model.eval()
        hidden = torch.tensor([[1.0, -2.0, 3.0], [0.5, 1.5, -1.0]])
        expected = model._SoftHalting__compute_gate_logits(hidden)
        ...
        restored.eval()
        actual = restored._SoftHalting__compute_gate_logits(hidden)

        self.assertTrue(model.state_dict())
        torch.testing.assert_close(actual, expected)

    def test_backward_reaches_gate_parameters_and_hidden_input(self):
        model = self.preset(input_dim=3).build()
        model.eval()
        initial_hidden = torch.ones(1, 2, 3)
        learned_hidden = torch.tensor(
            [[[1.0, -2.0, 3.0], [0.5, 1.5, -1.0]]],
            requires_grad=True,
        )
        state, _ = model.update_halting_state(None, initial_hidden)
        state, output = model.update_halting_state(state, learned_hidden)
        final_output, ponder_loss = model.compute_output(
            state,
            learned_hidden,
            torch.zeros_like(learned_hidden),
            torch.ones(1, 2),
        )

        (output.sum() + final_output.sum() + ponder_loss).backward()
        ...
        self.assertIsNotNone(learned_hidden.grad)
        self.assertTrue(torch.any(learned_hidden.grad.abs() > 0))
