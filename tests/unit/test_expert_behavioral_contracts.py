...
class ExpertBehavioralContractTests(unittest.TestCase):
    ...
    def test_restored_moe_boundary_preserves_input_rows(self) -> None:
        model = MixtureOfExpertsModel(
            _routing_model_config(RoutingInitializationMode.LAYER)
        )
        inputs = torch.randn(3, 2, requires_grad=True)
        result = model(MixtureOfExpertsLayerState(hidden=inputs))
        self.assertEqual(result.hidden.shape, inputs.shape)
        result.hidden.sum().backward()
        self.assertTrue(torch.isfinite(inputs.grad).all())

    def test_ungrouped_unreduced_multi_route_output_remains_supported(self) -> None:
        ...
        result = model(
            MixtureOfExpertsLayerState(
                hidden=torch.randn(3, 2), probabilities=torch.ones(3, 2)
            )
        )
        self.assertEqual(tuple(result.hidden.shape), (6, 2))
