...
class TestDynamicBiasHandlers(unittest.TestCase):
    ...
    def test_bias_decay_state_dict_round_trip_continues_exact_schedule_state(self):
        ...
        self.assertEqual(source._decay_policy.warmup_step.item(), 2.0)
        self.assertEqual(source._decay_policy.decay_step.item(), 2.0)
        self.assertTrue(
            torch.equal(
                restored._decay_policy.warmup_step, source._decay_policy.warmup_step
            )
        )
        self.assertTrue(
            torch.equal(
                restored._decay_policy.decay_step, source._decay_policy.decay_step
            )
        )
        self.assertTrue(
            torch.equal(
