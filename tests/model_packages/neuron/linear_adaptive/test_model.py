...
from models.neuron.linear_adaptive.presets import (
    _PRESET_DEFINITIONS,
    Experiment,
    ExperimentPreset,
)
...
class TestNeuronLinearAdaptiveModel(unittest.TestCase):
    ...
    def test_removed_terminal_z_axis_offset_override_is_rejected(self):
        with self.assertRaisesRegex(TypeError, "cluster_terminal_z_axis_offset"):
            model_package("neuron/linear_adaptive").bind_runtime_defaults(
    ...
    def test_builder_wraps_package_local_hidden_block(self):
        cfg = NeuronLinearAdaptiveConfigBuilder().build()
    ...
    def test_flat_cluster_capacity_reaches_final_configuration(self):
        ...
        configuration = model_package("neuron/linear_adaptive").build_configuration(
            config_overrides={
                "hidden_dim": 12,
