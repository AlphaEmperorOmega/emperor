...
from models.transformer.expert_linear_adaptive.model import Model
...
class TestTransformerExpertLinearAdaptiveModel(unittest.TestCase):
    ...
    def test_all_adaptive_expert_roles_and_routed_gradients(self):
        cfg = self._config(
            ...
            router_adaptive_weight_option=LowRankDynamicWeightConfig,
        ...
        controlled = Model(self._config(ExperimentPreset.RECURRENT_GATING_HALTING))
        self.assertTrue(
            any(isinstance(module, LinearLayer) for module in controlled.modules())
        )

    def test_baseline_lifecycle_and_low_rank_expert_signature_train(self):
        ...
        signature_cfg = self._config(ExperimentPreset.LOW_RANK_EXPERT_WEIGHT)
