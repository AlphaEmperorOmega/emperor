...
def _build_config(**runtime_defaults):
    runtime = model_package("transformer/linear_adaptive").bind_runtime_defaults(
        runtime_defaults
    )
    return TransformerLinearAdaptiveConfigBuilder(runtime=runtime).build()
...
class TestTransformerLinearAdaptiveModel(unittest.TestCase):
    ...
    def _config(self, preset=ExperimentPreset.BASELINE, dataset=None, **overrides):
        return model_package("transformer/linear_adaptive").presets.get_config(
            preset,
    ...
    def test_public_surface_catalog_identity_and_translation_task(self):
        ...
        experiment = Experiment(
            model_package=model_package("transformer/linear_adaptive")
        )
        self.assertEqual(
        ...
        self.assertIs(
            ...
            model_package("transformer/linear_adaptive"),
        )
        self.assertEqual(
    ...
    def test_all_presets_build_forward_and_keep_attention_roles(self):
        ...
        presets = model_package("transformer/linear_adaptive").presets
    ...
    def test_baseline_builds_for_both_multi30k_directions(self):
        for dataset in self._datasets():
            with self.subTest(dataset=dataset.__name__):
                cfg = model_package("transformer/linear_adaptive").presets.get_config(
