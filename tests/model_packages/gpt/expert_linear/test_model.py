    ...
    def test_every_preset_executes_mixture_attention_and_expert_ff(self):
        for preset in ExperimentPreset:
            with self.subTest(preset=preset.name):
                ...
                self.assertTrue(
                    any(isinstance(m, MixtureOfAttentionHeads) for m in modules)
                )
                self.assertTrue(any(isinstance(m, MixtureOfExperts) for m in modules))
                self.assertFalse(any(isinstance(m, SelfAttention) for m in modules))
                self.assertEqual(
                    tuple(logits.shape),
                    (2, cfg.sequence_length, cfg.output_dim),
                )
    ...
    def test_attention_kv_modes_forward_backward(self):
        ...
        for name, preset, use_kv_experts, expect_bias in cases:
            with self.subTest(name=name):
                ...
                logits, auxiliary_loss = model(self._input_ids(cfg))
                attention = next(
                    m for m in model.modules() if isinstance(m, MixtureOfAttentionHeads)
                )
                self.assertEqual(
