...
class TestResidualModelFlagCatalogContract(unittest.TestCase):
        ...
        for package in discover_model_packages():
            ...
            with self.subTest(package=package.catalog_key):
                self.assertTupleEqual(
                    ...
                    tuple(suffix for suffix in _STACK_OPTION_SUFFIXES if package.identity.model_type in ('linears', 'transformer', 'bert', 'gpt', 'vit', 'experts', 'neuron') or suffix not in ("RESIDUAL_BLOCK_SIZE", "RESIDUAL_RMS_NORM_EPSILON")),
                )
    ...
    def test_every_package_local_factory_builds_only_supported_residual_stacks(
        ...
        for package in discover_model_packages():
            ...
            attention_enabled = package.identity.model_type in ('linears', 'transformer', 'bert', 'gpt', 'vit', 'experts', 'neuron')
