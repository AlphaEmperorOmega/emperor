...
def _parse_search(
    ...
    if spec.axes == ():
        raise InvalidRunRequest(
            "Training search requires at least one selected axis."
        )
    if (
    ...
    if spec.axes is None and budget.max_axes is not None:
        ...
        if len(semantic_axes) > budget.max_axes:
            raise PlanTooLarge(
                f"Training search accepts at most {budget.max_axes} selected "
                "axes."
            )
    ...
    for selection in selections:
        ...
        if axis is not None:
            ...
            locked_value = serialize_config_value(
        ...
        if existing_position is not None and implicit_full_search:
            continue
        raw_values = (
            default_values if selection.values is None else selection.values
        )
        if not raw_values:
            raise InvalidRunRequest(
        ...
        if locked:
            lock_is_unchanged = (
                selection.allow_custom_values
                and all(value == locked_value for value in serialized_values)
            )
            if not lock_is_unchanged:
                raise InvalidRunRequest(
                    f"Search axis '{axis_key}' is locked by preset "
                    f"'{preset_name}'."
                )
...
def _reject_conflicting_locks(
    ...
    raise InvalidRunRequest(
        f"Preset '{preset_name}' does not allow overriding locked fields: "
        f"{details}"
    )
...
def _planned_run_count(resolved: _ResolvedRequest) -> int:
    ...
    return sum(
        (search.prepared.selected_count if search is not None else 1)
        * dataset_count
