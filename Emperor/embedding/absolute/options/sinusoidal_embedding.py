...
class SinusoidalPositionalEmbedding(Module):
    def __init__(
        ...
        self.padding_idx = self._get_padding_idx(cfg)
    ...
    def _get_padding_idx(self, cfg: "PositionalEmbeddingConfig") -> int:
    ...
    def _get_embedding(
        ...
        embedding = self._maybe_add_odd_dim_padding(embedding, embedding_dim, num_embeddings)
    ...
    def _compute_embedding_tensor(
        ...
        frequency_exponents = torch.arange(half_dim, dtype=torch.float) * -frequency_scale
        ...
        embedding = torch.cat([torch.sin(scaled_positions), torch.cos(scaled_positions)], dim=1)
