...
class SamplerModel(Module):
    def __init__(
        ...
        self.num_experts = self.__get_router_range()
        self.router_model = self.cfg.get("router_model")
        self.sampler_model = self._init_sampler_model()

    def __get_router_range(self):
        router_model = self.cfg.get("router_model")
        return (
            router_model.output_dim
            if router_model
            else self.cfg.router_model_config.output_dim
        )

    def _init_sampler_model(self) -> SamplerBase:
        if self.sampler_inputs["top_k"] == 1:
            return SamplerSparse(self.cfg, self.overrides)
        elif self.sampler_inputs["top_k"] == self.num_experts:
