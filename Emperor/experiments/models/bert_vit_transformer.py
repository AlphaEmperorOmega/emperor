...
class BERTVITExperimentOptions(BaseOptions):
    BASE_VIT = 0
    ADAPTIVE_VIT = 1
...
class BERTVITModel(Module):
    def __init__(
        ...
        self.main_cfg: BERTVITModelConfig = self._resolve_main_config(self.cfg, cfg)
...
class BERTVITExperiment(Experiments):
    def __init__(
        self,
    ...
    def _get_model_type(self):
        return TransformerClassifierExperiment

    def train_model(
        self,
        model_config_options: BERTVITExperimentOptions = BERTVITExperimentOptions.BASE_VIT,
    ):
        preset = BERTVITExperimentPresets().get_config(model_config_options)
        self._set_model_config(preset)
        self._train_model(BERTVITModel, print_parameter_count_flag=True)


class BERTVITExperimentPresets:
    def get_config(
        self,
        model_config_options: BERTVITExperimentOptions = BERTVITExperimentOptions.BASE_VIT,
    ) -> "ModelConfig":
        match model_config_options:
            case BERTVITExperimentOptions.BASE_VIT:
                return self.base_linear_transformer_config()
            case BERTVITExperimentOptions.ADAPTIVE_VIT:
                return self.adaptive_linear_transformer_config()
            case _:
                raise ValueError(
                    "The specified option is not supported. Please choose a valid `BERTVITExperimentOptions`."
                )
    ...
    def base_linear_transformer_config(self) -> "ModelConfig":
        ...
        return ModelConfig(
            ...
            override_config=BERTVITModelConfig(
    ...
    def adaptive_linear_transformer_config(self) -> "ModelConfig":
        ...
        ATTN_DIAGONAL_OPTION = DynamicDiagonalOptions.DISABLED
        ATTN_BIAS_OPTION = DynamicBiasOptions.DYNAMIC_PARAMETERS
        ATTN_BEHAVIOUR_STACK_NUM_LAYERS = 2

        FF_BIAS_FLAG = True
        FF_NUM_LAYERS = 2
        FF_GENERATOR_DEPTH = DynamicDepthOptions.DEPTH_OF_TWO
        FF_DIAGONAL_OPTION = DynamicDiagonalOptions.DISABLED
