...
The public ASGI import target is stable at `workbench.backend.api:app`. Code that
needs a configured app should import `create_app` and `WorkbenchApiSettings` from
`workbench.backend.api`; `workbench.backend.settings` remains a stable settings
compatibility import backed by `workbench.backend.core.config`.

HTTP routes are mounted at root paths such as `/health`, `/models`, and
`/training/jobs`. The `workbench.backend.api.v1` package is an internal FastAPI
