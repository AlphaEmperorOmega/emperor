...
def log_run_to_payload(run: LogRun) -> dict[str, Any]:
    return {
        ...
        **model_identity_payload_from_id(run.model),
        "preset": run.preset,
        ...
        "experiments": [
            {
                ...
                "models": [
                    {
                        **model_identity_payload_from_id(facet.model),
                        "count": facet.count,
                    }
...
def _scalar_point_to_payload(point: LogScalarPoint) -> dict[str, Any]:
...
def _histogram_to_payload(histogram: LogHistogram) -> dict[str, Any]:
    return {
        "tag": histogram.tag,
        "step": histogram.step,
...
def _monitor_image_to_payload(image: LogMonitorImage) -> dict[str, Any]:
    return {
        "tag": image.tag,
        "step": image.step,
...
def log_monitor_data_to_payload(data: LogMonitorData) -> dict[str, Any]:
    return {
        "jobId": data.job_id,
...
def _parameter_channel_to_payload(
    channel: LogParameterChannelStatus,
) -> dict[str, Any]:
    return {
...
def log_parameter_status_to_payload(status: LogParameterStatus) -> dict[str, Any]:
    return {
...
def _delete_candidate_to_payload(
    ...
    return {
        ...
        **model_identity_payload_from_id(candidate.model),
    ...
    *,
    ...
    return {
        ...
        "affected": {
            ...
            "datasets": datasets,
            "models": [model_identity_payload_from_id(model) for model in models],
            "presets": presets,
            "runIds": run_ids,
        },
