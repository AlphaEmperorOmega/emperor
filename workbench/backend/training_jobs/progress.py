"""Training Job progress-event persistence and process-local read cache."""
...
class TrainingProgressStore:
    ...
    def read_snapshot_uncached(
        ...
        """Read a released terminal job without retaining process-local state."""
        with self._lock:
            self._cache.pop(job.progress_path, None)
            snapshot = self._read_snapshot(job)
            self._cache.pop(job.progress_path, None)
            return snapshot
    ...
    def evict(self, job: TrainingJobRecord) -> None:
        """Drop process-local cached progress for a terminal Training Job."""
        with self._lock:
            self._cache.pop(job.progress_path, None)
