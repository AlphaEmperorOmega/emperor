...
from workbench.backend import training_worker
from workbench.backend.inspector.errors import InspectorError
from workbench.backend.inspector.search import parse_training_search
from workbench.backend.training_events import NeuronClusterGrowthCallback
from workbench.backend.training_worker import search_mode_from_parsed_search
...
class TrainingWorkerMaterializedRunConversionTests(unittest.TestCase):
    def materialized_runs(
        ...
        *,
        ...
        with (
            patch(
                "workbench.backend.training_worker.load_model_parts", return_value=parts
            ),
            patch(
...
class TrainingWorkerPayloadProgressTests(unittest.TestCase):
    ...
    def test_worker_loads_payload_and_writes_started_and_completed_events(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                ...
                training_worker.WORKBENCH_PROGRESS_STEP_INTERVAL,
            )
    ...
    def test_worker_uses_materialized_plan_instead_of_search_mode(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "workbench.backend.training_worker.parse_training_search",
