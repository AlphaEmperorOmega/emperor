...
describe("query key factories", () => {
  ...
  it("preserves workbench query key shapes", () => {
    expect(workbenchQueryKeys.health()).toEqual(["health"]);
    expect(workbenchQueryKeys.models()).toEqual(["models"]);
    expect(workbenchQueryKeys.presets("linears", "linear")).toEqual([
      "presets",
      "linears",
      "linear",
    ]);
    expect(workbenchQueryKeys.datasets("linears", "linear")).toEqual([
      "datasets",
      "linears",
      "linear",
    ]);
    expect(workbenchQueryKeys.monitors("linears", "linear")).toEqual([
      "monitors",
      "linears",
      "linear",
    ]);
    expect(workbenchQueryKeys.configSchema("linears", "linear", "baseline")).toEqual([
      "config-schema",
      "linears",
      "linear",
      "baseline",
    ...
    expect(
      workbenchQueryKeys.searchSpace("linears", "linear", "baseline", [
        "post-norm",
        "baseline",
      ]),
    ).toEqual([
    ...
    expect(
      workbenchQueryKeys.historicalSummaryInspection("linear", "baseline", "Mnist"),
    ).toEqual(["inspect", "historical-summary", "linear", "baseline", "Mnist"]);
    expect(workbenchQueryKeys.previewInspections()).toEqual(["inspect", "preview"]);
    expect(workbenchQueryKeys.previewInspection("request-key")).toEqual([
      "inspect",
      "preview",
      "request-key",
    ]);
    expect(workbenchQueryKeys.configSnapshots("linears", "linear")).toEqual([
      "config-snapshots",
      "linears",
      "linear",
    ]);
    expect(workbenchQueryKeys.configSnapshotLibrary()).toEqual([
