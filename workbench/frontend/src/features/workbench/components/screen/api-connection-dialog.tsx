...
import { useApiConnection } from "@/features/workbench/providers/workbench-providers";
...
export function ApiConnectionDialog({ onClose }: { onClose: () => void }) {
  ...
  const handleUse = (event: FormEvent<HTMLFormElement>) => {
    ...
    const normalizedUrl = normalizeWorkbenchApiBaseUrl(inputValue);
    if (!normalizedUrl) {
      setError(
      ...
      header={
        <header className="flex items-start justify-between gap-3 border-b border-line-soft px-4 py-3 sm:px-5">
          <div className="min-w-0">
            ...
            >
              Choose the browser API base URL and allow this Workbench origin in
        ...
        >
          ...
          <div className="grid gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
            <Input
              ...
              placeholder={WORKBENCH_API_BASE_URL}
            />
