...
function previewRequest(preset: string): InspectionPreviewRequest {
  return {
    modelType: "linears",
    model: "linear",
    preset,
...
describe("useInspectionPreviewState", () => {
  ...
  it("aborts a superseded inspection without exposing cancellation as an error", async () => {
    ...
    const { result } = renderHook(() => useInspectionPreviewState(), {
      wrapper: ({ children }: { children: ReactNode }) => (
        <QueryClientProvider client={queryClient}>{children}</QueryClientProvider>
      ),
    });

    act(() => result.current.ensure(previewRequest("baseline")));
    await waitFor(() => expect(pendingByPreset.has("baseline")).toBe(true));

    act(() => result.current.ensure(previewRequest("fast")));
    ...
    await waitFor(() => expect(result.current.response?.preset).toBe("fast"));
