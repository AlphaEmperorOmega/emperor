...
export function useLogsChartViewModel(state: LogsChartsInput) {
  ...
  const previousScalarSeriesRef = useRef<LogScalarSeries[]>([]);
  const previousTrainValidationScalarSeriesRef = useRef<LogScalarSeries[]>([]);
  const previousConfusionMatrixScalarSeriesRef = useRef<LogScalarSeries[]>([]);
  useLayoutEffect(() => {
    const scalarContentScopeIsMeaningful =
      state.visibleRunIds.length > 0 && retainedScalarTags.length > 0;
    if (scalarContentScopeIsMeaningful && !scalarSeriesReplacementPending) {
      previousScalarSeriesRef.current = queriedScalarSeries;
    } else if (queriedScalarSeries.length > 0) {
      previousScalarSeriesRef.current = retainCompatibleScalarSeries({
        current: queriedScalarSeries,
        previous: previousScalarSeriesRef.current,
        retainPrevious: true,
        runIds: state.visibleRunIds,
        tags: retainedScalarTags,
      });
    }
    const comparisonContentScopeIsMeaningful =
      trainValidationComparisonContentVisible &&
      state.visibleRunIds.length > 0 &&
      selectedTrainValidationScalarTags.length > 0;
    if (
      comparisonContentScopeIsMeaningful &&
      !trainValidationSeriesReplacementPending
    ) {
      previousTrainValidationScalarSeriesRef.current =
        queriedTrainValidationScalarSeries;
    } else if (queriedTrainValidationScalarSeries.length > 0) {
      previousTrainValidationScalarSeriesRef.current =
        retainCompatibleScalarSeries({
          current: queriedTrainValidationScalarSeries,
          previous: previousTrainValidationScalarSeriesRef.current,
          retainPrevious: true,
          runIds: state.visibleRunIds,
          tags: selectedTrainValidationScalarTags,
        });
    }
    const confusionContentScopeIsMeaningful =
      confusionMatrixScalarContentVisible &&
      state.visibleRunIds.length > 0 &&
      state.confusionMatrixRateTags.length > 0;
    if (
      confusionContentScopeIsMeaningful &&
      !confusionMatrixSeriesReplacementPending
    ) {
      previousConfusionMatrixScalarSeriesRef.current =
        queriedConfusionMatrixScalarSeries;
    } else if (queriedConfusionMatrixScalarSeries.length > 0) {
      previousConfusionMatrixScalarSeriesRef.current =
        retainCompatibleScalarSeries({
          current: queriedConfusionMatrixScalarSeries,
          previous: previousConfusionMatrixScalarSeriesRef.current,
          retainPrevious: true,
          runIds: state.visibleRunIds,
          tags: state.confusionMatrixRateTags,
        });
